...
def inference(model,
    ...
    if trans_info is not None:
        ...
        if not use_multilabel:
            pred = paddle.argmax(logit, axis=1, keepdim=True, dtype='int64')
        else:
...
def aug_inference(model,
    ...
    if not use_multilabel:
        pred = paddle.argmax(final_logit, axis=1, keepdim=True, dtype='int64')
    else:
        pred = (F.sigmoid(final_logit) > 0.5).astype('int64')
