...
def lovasz_softmax_flat(probas, labels, classes='present'):
    ...
    for c in classes_to_sum:
        ...
        # print(errors.shape)
        # errors,pad_length = pad_to_power_of_two(errors)
        # print(pad_length)
        # print(errors.shape)
        if hasattr(paddle, "_legacy_C_ops"):
            ...
            errors_sorted, perm = paddle._C_ops.argsort(errors, 'axis', 0,
                                                        'descending', True)
        # errors_sorted = errors_sorted[:-pad_length]
        # perm = perm[:-pad_length]
        errors_sorted.stop_gradient = False
...
def pad_to_power_of_two(x):
    ...
    while m < n:
        m *= 2
    pad_length = m - n
    padding_tensor = paddle.full([pad_length], -1, dtype=x.dtype)
    padded_x = paddle.concat([x, padding_tensor], axis=0)
    return padded_x,pad_length