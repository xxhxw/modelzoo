...
def evaluate(data, model, onset_threshold=0.5, frame_threshold=0.5, save_path=None):
    ...
    for label in data:
        if isinstance(model,DP):
            pred, losses = model._layers.run_on_batch(label)
        else:
        ...
        for key, value in pred.items():
            paddle.nn.functional.relu_(value.squeeze_(0))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', default='cpu')#'gpu' if paddle.device.is_compiled_with_cuda() else 'cpu')
