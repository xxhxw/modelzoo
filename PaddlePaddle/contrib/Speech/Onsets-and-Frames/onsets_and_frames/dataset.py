...
class PianoRollAudioDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.sequence_length is not None:
            ...
            result['velocity'] = data['velocity'].astype(paddle.float32)

        result['audio'] = result['audio'].astype(paddle.float32).divide(paddle.to_tensor(32768.0,place='cpu'))
        result['onset'] = (result['label'] == 3).astype(paddle.float32)
        result['offset'] = (result['label'] == 1).astype(paddle.float32)
        result['frame'] = (result['label'] > 1).astype(paddle.float32)
        result['velocity'] = result['velocity'].astype(paddle.float32).divide(paddle.to_tensor(128.0,place='cpu'))
    ...
    def load(self, audio_path, tsv_path):
        ...
        """
        saved_data_path = audio_path.replace('.flac', '.pdparams').replace('.wav', '.pdparams')
        if os.path.exists(saved_data_path):
            return paddle.load(saved_data_path)

        audio, sr = soundfile.read(audio_path, dtype='int16')
        assert sr == SAMPLE_RATE

        audio = paddle.to_tensor(audio,dtype = paddle.int32, place='cpu') # 智障飞桨int16某些算子不支持GPU
        audio_length = len(audio)

        n_keys = MAX_MIDI - MIN_MIDI + 1
        n_steps = (audio_length - 1) // HOP_LENGTH + 1

        label = paddle.zeros([n_steps, n_keys,]).cpu().astype(paddle.int32)
        velocity = paddle.zeros([n_steps, n_keys,]).cpu().astype(paddle.int32)
        ...
        for onset, offset, note, vel in midi:
            ...
            if onset_right<=frame_right:
                label[onset_right:frame_right, f] = 2
            label[frame_right:offset_right, f] = 1
            velocity[left:frame_right, f] = int(vel)

        data = dict(path=audio_path, audio=audio, label=label, velocity=velocity)
        paddle.save(data, saved_data_path)
        return data


class MAESTRO(PianoRollAudioDataset):

    def __init__(self, path='datas/MAESTRO', groups=None, sequence_length=None, seed=42, device=DEFAULT_DEVICE):
...
class MAPS(PianoRollAudioDataset):
    def __init__(self, path='datas/MAPS', groups=None, sequence_length=None, seed=42, device=DEFAULT_DEVICE):
    ...
    def files(self, group):
        ...
        assert(all(os.path.isfile(tsv) for tsv in tsvs)),tsvs
