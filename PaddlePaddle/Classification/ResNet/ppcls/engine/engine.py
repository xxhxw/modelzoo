...
from ppcls.utils.tcap_logger import init_tcap_logger
from ppcls.utils import init_tcap_logger
import time
...
class Engine(object):
    def __init__(self, config, mode="train"):
        ...
        if self.config['Global']['device'] == 'gpu':
            ...
            self.amp_list_black = {}
            self.amp_list_white ={'conv2d','matmul_v2','relu','pool2d'}
        else:
            self.amp_list_black = {'pool2d'}
            self.amp_list_white ={'conv2d','matmul_v2','relu',}
        # set seed
        ...
        self.teco_logger = init_tcap_logger(self.config["Arch"]["name"])
        print_config(config)
        ...
        if self.config["Global"]["distributed"]:
            ...
            self.model = paddle.DataParallel(self.model, comm_buffer_size=110)
            if self.mode == 'train' and len(self.train_loss_func.parameters(
            )) > 0:
    ...
    def train(self):
        ...
        if self.config['Global']['FP64'] and self.config['Global']['device']=='cpu':
            self.model = self.model.to('cpu',dtype=paddle.float64)
        
        if self.config["Global"]['visualdl']:
            
            timestr = str(time.strftime("%m%d_%H%M%S"))
            if paddle.distributed.get_world_size() > 1:
                device_id = paddle.distributed.ParallelEnv().device_id
            else:
                device_id = 0
            tb_path = os.path.join(f"./visualdl/{timestr}", str(self.config["Global"]['device']),str(device_id))
            writer = LogWriter(logdir=tb_path)
        else:
            writer=None
            
        for epoch_id in range(best_metric["epoch"] + 1,
                              self.config["Global"]["epochs"] + 1):
            if self.config["Global"]['early_stop'] >0:
                if epoch_id >= self.config["Global"]['early_stop']:
                    print(f'early stop at epoch {epoch_id}')
                    break
            acc = 0.0
            
            self.train_dataloader.batch_sampler.set_epoch(best_metric["epoch"]+1)
            
            if self.config["Global"]["train"]:
                # for one epoch train
                self.train_epoch_func(self, epoch_id, print_batch_step,writer)
