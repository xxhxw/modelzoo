...
def fit_one_epoch(G_model_A2B_train, G_model_B2A_train, D_model_A_train, D_model_B_train, G_model_A2B, G_model_B2A, D_model_A, D_model_B, loss_history, 
                G_optimizer, D_optimizer_A, D_optimizer_B, BCE_loss, MSE_loss, epoch, epoch_step, gen, Epoch, sdaa, fp16, scaler, save_period, save_dir, photo_save_step, model_train_log_file,local_rank=0):
    G_total_loss    = 0
    D_total_loss_A  = 0
    D_total_loss_B  = 0
    ...
    for iteration, batch in enumerate(gen):
        ...
        with torch.no_grad():
            if sdaa:
                images_A, images_B, y_real, y_fake  = images_A.sdaa(local_rank), images_B.sdaa(local_rank), y_real.sdaa(local_rank), y_fake.sdaa(local_rank)
        ...
        if local_rank == 0:
            ...
            pbar.update(1)
            model_train_log_file.write(f'Epoch:{epoch+1} G_loss:{G_loss.item()} D_loss_A:{D_loss_A.item()} D_loss_B:{D_loss_B.item()}\n')
