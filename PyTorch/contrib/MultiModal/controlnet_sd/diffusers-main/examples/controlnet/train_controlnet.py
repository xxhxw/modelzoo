...
def log_validation(
    ...
    image_logs = []
    inference_ctx = contextlib.nullcontext() if is_final_validation else torch.autocast("sdaa")
...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    if args.dataset_name is not None:
        ...
        if args.train_data_dir is not None:
            ...
            dataset = load_dataset(
                'json',
                data_files=args.train_data_dir+'/train.jsonl',
                cache_dir=args.cache_dir,
            )
    ...
    def preprocess_train(examples):
        ...
        images = [Image.open(os.path.join(args.train_data_dir, img_path)).convert("RGB") for img_path in examples[image_column]]
...
def main(args):
                ...
                for i, model in enumerate(models):
                    ...
                    for name, param in model.named_parameters():
                        if not param.is_contiguous():
                            # 克隆张量以确保它是连续的
                            setattr(model, name, param.contiguous())
                    
                    sub_dir = "controlnet"  # 或者根据实际情况选择不同的子目录
                    model.save_pretrained(os.path.join(output_dir, sub_dir))
    ...
    print("====,accelerater",type(accelerator.device))
    ...
    if args.allow_tf32:
        torch.backends.sdaa.matmul.allow_tf32 = True
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if global_step >= args.max_train_steps:
                break
        filename = f'tww/controlnet_sd/diffusers-main/examples/controlnet/loss_{epoch}.txt'
...
'''accelerate launch --multi_gpu train_controlnet.py \
 ...
 --resolution=512 \
 --learning_rate=1e-6 \
 --validation_image "/data/datasets/20241122/fusing-fill50k/conditioning_images/0.png" "/data/datasets/20241122/fusing-fill50k/conditioning_images/1.png" "/data/datasets/20241122/fusing-fill50k/conditioning_images/2.png" \
 --validation_prompt "pale golden rod circle with old lace background" "light coral circle with white background" "aqua circle with light pink background" \
 --train_batch_size=1 \
 --num_train_epochs=3 \
