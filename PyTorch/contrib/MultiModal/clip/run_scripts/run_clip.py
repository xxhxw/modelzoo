...
if __name__ == '__main__':
    ...
    output_dir =  os.path.join(current_file_directory, '../logs/clip_multi')
    output_dir_single = os.path.join(current_file_directory, '../logs/clip_single')
    print(output_dir)
    if nnode > 1:
        raise Exception("Recent task do not support nnode > 1. Set --nnode=1 !")

    if 'clip' not in model_name:
        raise ValueError('please use clip model')
