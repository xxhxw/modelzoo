...
class EvalCallback():
    ...
    def on_epoch_end(self, epoch, model_eval,epoch_step,json_logger):
        ...
        if epoch % self.period == 0 and self.eval_flag:
            ...
            i_features = []
            t_features = []
            for iteration, batch in enumerate(self.gen_val):
                images, texts = batch
                with torch.no_grad():
                    if self.cuda:
            ...
            num_text    = len(texts)
            for i in range(0, num_text, self.batch_size):
            ...
            json_logger.log(
                step=(epoch, epoch_step),
                data={
                    "txt_r1": itm_results['txt_r1'],
                    "txt_r5": itm_results['txt_r5'],
                    "txt_r_mean": itm_results['txt_r_mean'],
                    "img_r1": itm_results['img_r1'],
                    "img_r5": itm_results['img_r5'],
                    "img_r_mean": itm_results['img_r_mean'],
                    "r_mean": itm_results['r_mean']
                },
