...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    dists = torch.cat([dists, torch.Tensor([1e10]).sdaa().expand(dists[...,:1].shape)], -1)  # [N_rays, N_samples]
...
def config_parser():
    ...
    parser.add_argument('--model_train_log', type=bool,default=True, 
                        help='config file path')
    ...
    parser.add_argument("--training_time",   type=int, default=2, 
                        help='hours')
...
def train():
    ...
    #if rank == 0:
        #只在主进程中保存
    if args.model_train_log and rank == 0:
        #model_train_log_path = os.path.join(basedir, expname, f'model_train_log_rank_{rank}.txt')
    ...
    for i in trange(start, N_iters):
        time0 = time.time()
        if (time0 - start_time) > args.training_time*3600:
            sys.exit(1)
        ...
        if i%args.i_print==0:
            ...
            if args.model_train_log and rank == 0:
                model_train_log_file.write(f'[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}\n')
                model_train_log_file.flush()
            '''if args.model_train_log and rank == 1:
                model_train_log_file.write(f'[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}\n')
                model_train_log_file.flush()
            if args.model_train_log and rank == 2:
                model_train_log_file.write(f'[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}\n')
                model_train_log_file.flush()
            if args.model_train_log and rank == 3:
                model_train_log_file.write(f'[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}\n')
                model_train_log_file.flush()'''
        """
...
if __name__=='__main__':
    #torch.sdaa.set_default_dtype('torch.sdaa.FloatTensor')
    # DDP backend初始化
    # rank号和device一一对应，进行set device
    # 需要在初始化ProcessGroup之前进行set device，tccl限制
    torch.set_default_device('sdaa')
    train()
