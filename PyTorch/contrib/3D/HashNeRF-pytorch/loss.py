...
def total_variation_loss(embeddings, min_resolution, max_resolution, level, log2_hashmap_size, n_levels=16):
    ...
    '''if torch.isnan(embeddings).any():
        print("发现 embeddings 中有 NaN")
    if torch.isinf(embeddings).any():
        print("发现 embeddings 中有 inf")'''
    ...
    '''if torch.isnan(cube_embeddings).any():
        print("发现 cube_embeddings 中有 NaN")
    if torch.isinf(cube_embeddings).any():
        print("发现 cube_embeddings 中有 inf")
        #print("cube_embeddings",cube_embeddings)'''
        # PyTorch 1.8+ 内置方法（推荐）
    #cube_embeddings = torch.nan_to_num(cube_embeddings, nan=1e-6, posinf=None, neginf=None)
    #cube_embeddings = torch.clamp(cube_embeddings, -1e6, 1e6)
    ...
    #tv_x,tv_y,tv_z = tv_x.to("sdaa"),tv_y.to('sdaa'),tv_z.to('sdaa')
