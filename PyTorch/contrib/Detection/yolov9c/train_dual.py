...
from argument import parse_opt as parse_opt

def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    device=torch.device('sdaa')
    #torch.sdaa.set_device('sdaa')
    ...
    if pretrained:
        ...
        ckpt = torch.load(weights, map_location='cpu')  # load checkpoint to CPU to avoid sdaa memory leak
        model = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to('sdaa')  # create
        ...
        model = Model(cfg, ch=3, nc=nc, anchors=hyp.get('anchors')).to('sdaa')  # create
    ...
    # DP mode
    if 'sdaa' and RANK == -1 and torch.sdaa.device_count() > 1:
        LOGGER.warning('WARNING ⚠️ DP not recommended, use torch.distributed.run for best DDP Multi-GPU results.')
        model = torch.nn.DataParallel(model)

    # SyncBatchNorm
    if opt.sync_bn and 'sdaa' and RANK != -1:
        model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model).to('sdaa')
    ...
    model.class_weights = labels_to_class_weights(dataset.labels, nc).to('sdaa') * nc  # attach class weights
    model.names = names
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        mloss = torch.zeros(3, device='sdaa')  # mean losses
        if RANK != -1:
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.sdaa.amp.autocast( ):
                ...
                print(imgs.device)
                #print(model.device)
                loss, loss_items = compute_loss(pred, targets.to('sdaa'))  # loss scaled by batch_size
                if RANK != -1:
            ...
            if f.exists():
                ...
                if f is best:
                    ...
                    results, _, _ = validate.run(
                        ...
                        batch_size=batch_size // WORLD_SIZE * 2,
                        imgsz=imgsz,
                        model=attempt_load(f, 'sdaa').float(),
                        single_cls=single_cls,
                        dataloader=val_loader,
                        save_dir=save_dir,
                        save_json=is_coco,
...
def main(opt, callbacks=Callbacks()):
    ...
    # DDP mode
    device = select_device('sdaa', batch_size=opt.batch_size)
    if LOCAL_RANK != -1:
        ...
        assert torch.sdaa.device_count() > LOCAL_RANK, 'insufficient sdaa devices for DDP command'
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(opt.evolve):  # generations to evolve
            ...
            # Train mutation
            results = train(hyp.copy(), opt, 'sdaa', callbacks)
            callbacks = Callbacks()
