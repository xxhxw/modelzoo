...
async def completed(trace_name='',
                    name='',
                    sleep_interval=0.05,
                    streams: List[torch.sdaa.Stream] = None):
    """Async context manager that waits for work to complete on given SDAA
    streams."""
    if not torch.sdaa.is_available():
        yield
        return

    stream_before_context_switch = torch.sdaa.Stream()
    if not streams:
    ...
    end_events = [
        torch.sdaa.Event(enable_timing=DEBUG_COMPLETED_TIME) for _ in streams
    ]
    ...
    try:
        ...
        current_stream = torch.sdaa.Stream()
        ...
        logger.debug('%s %s completed: %s streams: %s', trace_name, name,
                     are_done, streams)
        with torch.sdaa.Stream(stream_before_context_switch):
...
async def concurrent(streamqueue: asyncio.Queue,
    ...
    initial_stream = torch.sdaa.Stream()

    with torch.sdaa.Stream(initial_stream):
        stream = await streamqueue.get()
        assert isinstance(stream, torch.sdaa.Stream)
