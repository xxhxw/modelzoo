...
def init_dist(launcher,
    ...
    Args:
        ...
        backend (str): Communication Backends. Supported backends are 'tccl',
            'gloo' and 'mpi'. Defaults to 'tccl'.
    ...
    if timeout is not None:
        ...
        try:
            ...
            raise TypeError(
                ...
                f"dist_cfg=dict(backend='tccl', timeout=1800)") from exception
...
def _init_dist_pytorch(backend, init_backend='torch', **kwargs) -> None:
    ...
    Args:
        backend (str): Backend of torch.distributed. Supported backends are
            'tccl', 'gloo' and 'mpi'. Defaults to 'tccl'.
        **kwargs: keyword arguments are passed to ``init_process_group``.
    """
