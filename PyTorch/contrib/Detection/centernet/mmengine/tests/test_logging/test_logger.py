...
@patch('torch.sdaa.device_count', lambda: 4)
def test_get_device_id():

    @contextmanager
    def patch_env(local_rank, sdaa_visible_devices):
        ori_local_rank = os.getenv('LOCAL_RANK', None)
        ori_sdaa_visible_devices = os.getenv('SDAA_VISIBLE_DEVICES', None)

        if local_rank is not None:
            os.environ['LOCAL_RANK'] = local_rank
        if sdaa_visible_devices is not None:
            os.environ['SDAA_VISIBLE_DEVICES'] = sdaa_visible_devices
        yield
        if ori_local_rank is not None:
            os.environ['LOCAL_RANK'] = ori_local_rank
        elif 'LOCAL_RANK' in os.environ:
            os.environ.pop('LOCAL_RANK')
        if ori_sdaa_visible_devices is not None:
            os.environ['SDAA_VISIBLE_DEVICES'] = ori_sdaa_visible_devices
        elif 'SDAA_VISIBLE_DEVICES' in os.environ:
            os.environ.pop('SDAA_VISIBLE_DEVICES')

    # sdaa is not available and local_rank is not set
    with patch('torch.sdaa.is_available', lambda: False), \
         patch_env(None, '0,1,2,3'):
        assert _get_device_id() == 0

    # sdaa is not available and local_rank is set
    with patch('torch.sdaa.is_available', lambda: False), \
         patch_env('1', '0,1,2,3'):
        assert _get_device_id() == 1

    # SDAA_VISIBLE_DEVICES will not influence non-sdaa device
    with patch('torch.sdaa.is_available', lambda: False), \
         patch_env('1', '0,100,2,3'):
        assert _get_device_id() == 1

    # sdaa is available and local_rank is not set
    with patch('torch.sdaa.is_available', lambda: True), \
         patch_env(None, '0,1,2,3'):
        assert _get_device_id() == 0

    # sdaa is available and local_rank is set
    with patch('torch.sdaa.is_available', lambda: True), \
         patch_env('2', '0,1,2,3'):
        assert _get_device_id() == 2

    # SDAA_VISIBLE_DEVICES worked
    with patch('torch.sdaa.is_available', lambda: True), \
         patch_env('2', '0,1,3,5'):
