...
from mmengine.device import is_sdaa_available
...
class TestEmptyCacheHook(RunnerTestCase):
    ...
    @pytest.mark.skipif(
        not is_sdaa_available(), reason='sdaa should be available')
    def test_with_runner(self):
        with patch('torch.sdaa.empty_cache') as mock_empty_cache:
            ...
            # Call `torch.sdaa.empty_cache` after each epoch:
        ...
        with patch('torch.sdaa.empty_cache') as mock_empty_cache:
            ...
            # Call `torch.sdaa.empty_cache` after/before each epoch:
