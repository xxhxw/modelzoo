...
@pytest.mark.skipif(not torch.sdaa.is_available(), reason='test requires GPU')
def test_roipool():
    rt = pytest.importorskip('onnxruntime')
    ...
    for case in inputs:
        np_input = np.array(case[0], dtype=np.float32)
        np_rois = np.array(case[1], dtype=np.float32)
        input = torch.from_numpy(np_input).sdaa()
        rois = torch.from_numpy(np_rois).sdaa()
...
def test_border_align():
    ...
    model = BorderAlign(2)
    input = torch.rand(1, 8, 2, 2).sdaa()
    boxes = torch.rand(1, 4, 4).sdaa()
...
def test_carafe():
    ...
    feat = torch.randn(2, 64, 3, 3, device='sdaa').double()
    mask = torch.randn(2, 100, 6, 6, device='sdaa').sigmoid().double()
...
def test_deform_conv():
    ...
    x = torch.randn(1, 2, 4, 4, device='sdaa')
    _test_symbolic(
        DeformConv2dPack(2, 4, 3, 1, 1).sdaa(), (x, ), 'MMCVDeformConv2d')
...
def test_modulated_deform_conv():
    ...
    x = torch.randn(1, 2, 4, 4, device='sdaa')
    _test_symbolic(
        ModulatedDeformConv2dPack(2, 4, 3, 1, 1).sdaa(), x,
...
def test_deform_roi_pool():
    ...
    x = torch.tensor([[[[1., 2.], [3., 4.]]]], device='sdaa')
    rois = torch.tensor([[0., 0., 0., 1., 1.]], device='sdaa')
    output_c = x.size(1)
    pool_h = 2
    pool_w = 2
    spatial_scale = 1.0
    ...
    model = DeformRoIPoolPack((pool_h, pool_w),
                              ...
                              sampling_ratio=sampling_ratio).sdaa()
...
def test_masked_conv():
    ...
    x = torch.rand(1, 2, 4, 4, device='sdaa')
    mask = torch.rand(1, 4, 4, device='sdaa')
    _test_symbolic(
        MaskedConv2d(2, 4, 3, 1, 1).sdaa(), (x, mask), 'MMCVMaskedConv2d')
...
def test_pr_roi_pool():
    ...
    x = torch.tensor([[[[1., 2.], [3., 4.]]]], device='sdaa')
    rois = torch.tensor([[0., 0., 0., 1., 1.]], device='sdaa')
    model = PrRoIPool((pool_h, pool_w), spatial_scale).sdaa()
...
def test_psa_mask():
    ...
    input = torch.rand(4, 16, 8, 8).sdaa()
    model = PSAMask('collect', (4, 4)).sdaa()
...
def test_roi_align_rotated():
    ...
    sampling_ratio = 2
    x = torch.tensor([[[[1., 2.], [3., 4.]]]], device='sdaa')
    rois = torch.tensor([[0., 0.5, 0.5, 1., 1., 0]], device='sdaa')
...
def test_roi_feaeture_align():
    ...
    feature = torch.rand(1, 1, 2, 2, device='sdaa')
    bbox = torch.rand(1, 2, 2, 5, device='sdaa')
