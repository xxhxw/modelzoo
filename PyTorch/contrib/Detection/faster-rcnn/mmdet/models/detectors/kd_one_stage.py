...
class KnowledgeDistillationSingleStageDetector(SingleStageDetector):
    ...
    def sdaa(self, device: Optional[str] = None) -> nn.Module:
        """Since teacher_model is registered as a plain object, it is necessary
        to put the teacher model to sdaa when calling ``sdaa`` function."""
        self.teacher_model.sdaa(device=device)
        return super().sdaa(device=device)
