...
class TwoStageDetector(BaseDetector):
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        if self.with_rpn:  
            ...
            #     on_trace_ready=torch.profiler.tensorboard_trace_handler(dir_name='/root/profilebool/'),
            ...
            #     with_stack=True
            #     ) as prof:  
                # with torch.profiler.record_function("train_cfg"):
                    proposal_cfg = self.train_cfg.get('rpn_proposal',
                                                    self.test_cfg.rpn)
                # with torch.profiler.record_function("deepcopy"):
                    rpn_data_samples = copy.deepcopy(batch_data_samples)
                    # set cat_id of gt_labels to 0 in RPN
                # with torch.profiler.record_function("rpn_data_samples"):
                    for data_sample in rpn_data_samples:
                        data_sample.gt_instances.labels = \
                            torch.zeros_like(data_sample.gt_instances.labels)
                    # 耗时很长
                # with torch.profiler.record_function("rpn_head"):
                    rpn_losses, rpn_results_list = self.rpn_head.loss_and_predict(
                        x, rpn_data_samples, proposal_cfg=proposal_cfg)
                        # avoid get same name with roi_head loss    
                    keys = rpn_losses.keys()
                # with torch.profiler.record_function("rpn_losses"):
                    for key in list(keys):
                        if 'loss' in key and 'rpn' not in key:
                            rpn_losses[f'rpn_{key}'] = rpn_losses.pop(key)
                    losses.update(rpn_losses)
            # print(prof.key_averages(group_by_stack_n=5).table(sort_by="self_cpu_time_total", row_limit=10))
            # prof.export_chrome_trace('/root/profile.json')
        else:
            assert batch_data_samples[0].get('proposals', None) is not None
