...
@contextmanager
def _ignore_torch_sdaa_oom():
    """A context which ignores SDAA OOM exception from pytorch.
    ...
    try:
        ...
        if 'SDAA out of memory. ' in str(e):
            pass
        else:
            raise
...
class AvoidOOM:
    """Try to convert inputs to FP16 and CPU if got a PyTorch's SDAA Out of
    Memory error. It will do the following steps:

        1. First retry after calling `torch.sdaa.Event()`.
    ...
    Args:
        to_cpu (bool): Whether to convert outputs to CPU if get an OOM
            ...
            Defaults to True.
        test (bool): Skip `_ignore_torch_sdaa_oom` operate that can use
            lightweight data in unit test, only used in
            test unit. Defaults to False.

    Examples:
        >>> from mmdet.utils.memory import AvoidOOM
        >>> AvoidSDAAOOM = AvoidOOM()
        >>> output = AvoidOOM.retry_if_sdaa_oom(
        >>>     some_torch_function)(input1, input2)
        >>> # To use as a decorator
        >>> # from mmdet.utils import AvoidSDAAOOM
        >>> @AvoidSDAAOOM.retry_if_sdaa_oom
    ...
    def retry_if_sdaa_oom(self, func):
        """Makes a function retry itself after encountering pytorch's SDAA OOM
        error.
        ...
        def wrapped(*args, **kwargs):
            ...
            if self.to_cpu:
                ...
                warnings.warn('Cannot convert output to GPU due to SDAA OOM, '
                ...
                logger.warning('Cannot convert output to GPU due to '
                               'SDAA OOM, the output is on CPU now.')

                return func(*cpu_args, **cpu_kwargs)
            else:
                # may still get SDAA OOM error
                return func(*args, **kwargs)
