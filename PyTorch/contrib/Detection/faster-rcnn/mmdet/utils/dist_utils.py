...
def obj2tensor(pyobj, device='sdaa'):
...
def all_reduce_dict(py_dict, op='sum', group=None, to_float=True):
    ...
    """
    warnings.warn(
        'group` is deprecated. Currently only supports tccl backend.')
...
def sync_random_seed(seed=None, device='sdaa'):
