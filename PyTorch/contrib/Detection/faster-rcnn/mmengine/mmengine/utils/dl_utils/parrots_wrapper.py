...
def _get_sdaa_home() -> Optional[str]:
    """Obtain the path of SDAA home."""
    if TORCH_VERSION == 'parrots':
...
def _get_extension():
    ...
    if TORCH_VERSION == 'parrots':
        ...
        CppExtension = partial(Extension, sdaa=False)
        SDAAExtension = partial(Extension, sdaa=True)
    else:
        from torch_sdaa.utils.cpp_extension import (BuildExtension, CppExtension,
                                               SDAAExtension)
    return BuildExtension, CppExtension, SDAAExtension
