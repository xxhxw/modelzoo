...
def gather(data: Tensor,
    ...
    Note:
        ``TCCL`` backend does not support ``gather``.
    ...
    Args:
        data (Tensor): Tensor to be gathered. SDAA tensor is not supported.
...
def _broadcast_object_list(object_list: List[Any],
    ...
    # ``current_device`` is SDAA if backend is TCCL otherwise CPU device. In
    # the case it is not ``None`` we move the size and object tensors to be
    # broadcasted to this device.
    group_backend = get_backend(group)
    is_tccl_backend = group_backend == 'tccl'
    ...
    if is_hccl_backend:
        ...
        object_sizes_tensor = object_sizes_tensor.to(current_device)
    elif is_tccl_backend:
        # See note about using torch.sdaa.current_device() here in
        # docstring. We cannot simply use my_rank since rank == device is
        # not necessarily true.
        current_device = torch.device('sdaa', torch.sdaa.current_device())
        object_sizes_tensor = object_sizes_tensor.to(current_device)
    ...
    if is_tccl_backend or is_hccl_backend or is_cncl_backend:
...
def _all_gather_object(object_list: List[Any],
    ...
    is_mccl_backend = group_backend == 'mccl'
    if is_tccl_backend:
        # See note about using torch.sdaa.current_device() here in docstring.
...
def all_gather_object(data: Any,
    ...
    Note:
        For TCCL-based process groups, internal tensor representations
...
def gather_object(data: Any,
    ...
    Note:
        ``TCCL backend`` does not support ``gather_object``.
...
def _collect_results_device(result_part: list, size: int) -> Optional[list]:
    ...
    # gather all result part. Note that TCCL does not support gather so use
