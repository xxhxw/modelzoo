...
class AmpOptimWrapper(OptimWrapper):
    ...
    precision training based on torch_sdaa.amp.
    ...
    Args:
        loss_scale (float or str or dict): The initial configuration of
            `torch_sdaa.amp.GradScaler`. See more specific arguments
            introduction at `PyTorch AMP <https://pytorch.org/docs/stable/amp.html?highlight=gradscalertorch_sdaa.amp.GradScaler>`_ # noqa: E501
            Defaults to ``dynamic``.
    ...
    def __init__(self,
        ...
        assert digit_version(TORCH_VERSION) >= digit_version('1.6.0'), (
            '`torch_sdaa.amp` is only available when pytorch version >= 1.6')
        assert is_sdaa_available() or is_npu_available() or is_mlu_available(
