...
def skip_test_comile():
    ...
    if not torch.sdaa.is_available():
        return True
    try:
        model = nn.Sequential(nn.Conv2d(1, 1, 1), nn.BatchNorm2d(1)).sdaa()
        compiled_model = torch.compile(model)
        compiled_model(torch.ones(3, 1, 1, 1).sdaa())
    except Exception:
        return True
    else:
        return False
...
class CustomRunner(Runner):
    ...
    def __init__(self,
                 ...
                 resume=False,
                 launcher='none',
                 env_cfg=dict(dist_cfg=dict(backend='tccl')),
                 log_processor=None,
                 log_level='INFO',
                 visualizer=None,
                 default_scope=None,
...
class TestRunner(TestCase):
    ...
    def test_init(self):
        ...
        # 5. Test building multiple runners. In Windows, tccl could not be
        # available, and this test will be skipped.
        if torch.sdaa.is_available() and torch.distributed.is_tccl_available():
            cfg = copy.deepcopy(self.epoch_based_cfg)
    ...
    def test_val(self):
        ...
        if (digit_version(TORCH_VERSION) < digit_version('1.10.0')
                and not torch.sdaa.is_available()):
