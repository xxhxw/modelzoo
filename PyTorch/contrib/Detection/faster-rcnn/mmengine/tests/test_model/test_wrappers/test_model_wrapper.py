...
class TestDistributedDataParallel(MultiProcessTestCase):
    ...
    @unittest.skipIf(
        not torch.sdaa.is_available(), reason='sdaa should be available')
    def test_train_step(self):
        ...
        inputs = torch.randn(1, 3, 1, 1).sdaa() * self.rank * 255
...
class TestMMSeparateDistributedDataParallel(TestDistributedDataParallel):
    ...
    def test_init(self):
        ...
        model.act = nn.ReLU()
        ddp_model = MMSeparateDistributedDataParallel(model.sdaa())
...
@unittest.skipIf(
    torch.sdaa.device_count() < 2, reason='need 2 gpu to test fsdp')
@unittest.skipIf(
...
class TestMMFullyShardedDataParallel(MultiProcessTestCase):
    ...
    def _init_dist_env(self, rank, world_size):
        ...
        num_gpus = torch.sdaa.device_count()
        torch.sdaa.set_device(rank % num_gpus)
        torch_dist.init_process_group(
            backend='tccl', rank=rank, world_size=world_size)
    ...
    def test_train_step(self):
        ...
        model = ToyModel()
        fsdp_model = MMFullyShardedDataParallel(module=model.sdaa())
        ...
        fsdp_model = MMFullyShardedDataParallel(
            module=model.sdaa(), auto_wrap_policy=wrap_policy)
