...
class TestCorrelation:
    ...
    def _test_correlation(self, dtype=torch.float):
        ...
        input1 = torch.tensor(_input1, dtype=dtype).sdaa()
        input2 = torch.tensor(_input2, dtype=dtype).sdaa()
        ...
        # so we need to make a comparison for sdaa tensor
        # rather than cpu tensor
        gt_out = torch.tensor(_gt_out, dtype=dtype).sdaa()
