...
def filtered_lrelu(input: torch.Tensor,
    ...
    if use_custom_op and input.is_sdaa:
        return _filtered_lrelu_sdaa(
            up=up,
            down=down,
            padding=padding,
            gain=gain,
...
_filtered_lrelu_sdaa_cache: Dict = dict()


def _filtered_lrelu_sdaa(up: int = 1,
                         down: int = 1,
                         padding: int = 0,
                         gain: float = np.sqrt(2),
                         slope: float = 0.2,
                         clamp: Optional[Union[float, int]] = None,
                         flip_filter: bool = False):
    """Fast SDAA implementation of `filtered_lrelu()` using custom ops.
    ...
    if key in _filtered_lrelu_sdaa_cache:
        return _filtered_lrelu_sdaa_cache[key]
    ...
    class FilteredLReluCuda(torch.autograd.Function):
        ...
        def forward(ctx, input, filter_up, filter_down, bias, si, sx, sy):
            ...
            if input.dtype in [torch.float16, torch.float32]:
                if torch.sdaa.current_stream(
                        input.device) != torch.sdaa.default_stream(
                            input.device):
                    warnings.warn(
                        'filtered_lrelu called with non-default sdaa stream '
            ...
            if return_code < 0:
                warnings.warn(
                    ...
                    'optimized SDAA kernel, using generic fallback',
                    RuntimeWarning)
    ...
    # Add to cache.
    _filtered_lrelu_sdaa_cache[key] = FilteredLReluCuda
