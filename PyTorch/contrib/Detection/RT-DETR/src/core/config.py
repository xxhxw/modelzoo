...
class BaseConfig(object):
    ...
    def __init__(self) -> None:
        ...
        self._scaler :torch.sdaa.amp.grad_scaler.GradScaler = None
        ...
        if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
            device = f"sdaa:{int(os.environ['LOCAL_RANK'])}" if torch.sdaa.is_available() else 'cpu'
        else:
    ...
    @property
    def scaler(self) -> torch.sdaa.amp.grad_scaler.GradScaler:
        # [修改] 将 cuda 改为 sdaa 以适配 SDAA 设备
        if self._scaler is None and self.use_amp and torch.sdaa.is_available():
            # [修改] 将 GradScaler() 改为 torch.sdaa.amp.GradScaler()
            self._scaler = torch.sdaa.amp.GradScaler()
        return self._scaler
    
    @scaler.setter
    def scaler(self, obj: torch.sdaa.amp.grad_scaler.GradScaler):
        self._scaler = obj
