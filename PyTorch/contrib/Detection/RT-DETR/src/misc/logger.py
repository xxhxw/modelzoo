...
def all_gather(data):
    ...
    # [修改] 将 cuda 改为 sdaa 以适配 SDAA 设备
    tensor = torch.ByteTensor(storage).to("sdaa")

    # obtain Tensor size of each rank
    # [修改] 将 cuda 改为 sdaa 以适配 SDAA 设备
    local_size = torch.tensor([tensor.numel()], device="sdaa")
    size_list = [torch.tensor([0], device="sdaa") for _ in range(world_size)]
    ...
    tensor_list = []
    for _ in size_list:
        tensor_list.append(torch.empty((max_size,), dtype=torch.uint8, device="sdaa"))
    if local_size != max_size:
        padding = torch.empty(size=(max_size - local_size,), dtype=torch.uint8, device="sdaa")
