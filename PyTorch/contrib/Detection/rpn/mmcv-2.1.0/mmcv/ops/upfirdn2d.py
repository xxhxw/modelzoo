...
def upfirdn2d(input: torch.Tensor,
    ...
    if use_custom_op and input.device.type == 'sdaa':
        return _upfirdn2d_sdaa(
            up=up,
            down=down,
            padding=padding,
...
_upfirdn2d_sdaa_cache: Dict = dict()


def _upfirdn2d_sdaa(up: int = 1,
                    down: int = 1,
                    padding: Union[int, List[int]] = 0,
                    flip_filter: bool = False,
                    gain: Union[float, int] = 1):
    """Fast SDAA implementation of `upfirdn2d()` using custom ops.
    ...
    key = (upx, upy, downx, downy, padx0, padx1, pady0, pady1, flip_filter,
           gain)
    if key in _upfirdn2d_sdaa_cache:
        return _upfirdn2d_sdaa_cache[key]
    ...
    # Add to cache.
    _upfirdn2d_sdaa_cache[key] = Upfirdn2dCuda
