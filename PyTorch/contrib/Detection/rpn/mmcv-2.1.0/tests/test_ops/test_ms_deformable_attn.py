...
@pytest.mark.skipif(not IS_SDAA_AVAILABLE, reason='requires SDAA support')
def test_forward_equal_with_pytorch_double():
    ...
    output_sdaa = MultiScaleDeformableAttnFunction.apply(
        value.sdaa().double(), shapes.sdaa(), level_start_index.sdaa(),
        sampling_locations.sdaa().double(),
        attention_weights.sdaa().double(), im2col_step).detach().cpu()
    assert torch.allclose(output_sdaa, output_pytorch)
    max_abs_err = (output_sdaa - output_pytorch).abs().max()
    max_rel_err = ((output_sdaa - output_pytorch).abs() /
...
def test_forward_equal_with_autocast():
    ...
    with autocast(enabled=True):
        output_device = MultiScaleDeformableAttnFunction.apply(
            value.sdaa().type(dtype), shapes.sdaa(), level_start_index.sdaa(),
            sampling_locations.sdaa(), attention_weights.sdaa(),
            im2col_step).detach().cpu()
    assert torch.allclose(output_device, output_pytorch, rtol=1e-2, atol=1e-3)
    max_abs_err = (output_device - output_pytorch).abs().max()
    max_rel_err = ((output_device - output_pytorch).abs() /
