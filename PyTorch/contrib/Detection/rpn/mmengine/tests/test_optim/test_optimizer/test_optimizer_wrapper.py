...
def bf16_supported() -> bool:
    return (hasattr(torch.sdaa, 'is_bf16_supported')
            and torch.sdaa.is_bf16_supported())
...
@unittest.skipIf(not torch.sdaa.is_available(), reason='need gpu to test Apex')
class TestApexOptimWrapper(TestCase):

    def setUp(self) -> None:
        self.model = ToyModel().sdaa()
        self.optimizer = SGD(self.model.parameters(), lr=0.1)
    ...
    def test_step(self):
        ...
        with apex_optim_wrapper.optim_context(self.model):
            loss = self.model(torch.Tensor(1, 1, 1, 1).sdaa())
    ...
    def test_optim_context(self):
        apex_optim_wrapper = ApexOptimWrapper(
            optimizer=self.optimizer, opt_level='O1', loss_scale=1)
        with apex_optim_wrapper.optim_context(self.model):
            x = torch.randn(1, 1, 1, 1).sdaa()
            y = nn.Conv2d(1, 1, 1).sdaa()(x)
...
class TestAmpOptimWrapper(TestCase):
    ...
    @unittest.skipIf(
        not torch.sdaa.is_available(),
        reason='`torch_sdaa.amp` is only available when pytorch-gpu installed')
    def test_init(self):
