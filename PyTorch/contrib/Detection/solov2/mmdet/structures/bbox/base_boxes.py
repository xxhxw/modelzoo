...
IndexType = Union[slice, int, list, torch.LongTensor, torch.sdaa.LongTensor,
                  torch.BoolTensor, torch.sdaa.BoolTensor, np.ndarray]
...
class BaseBoxes(metaclass=ABCMeta):
    ...
    def sdaa(self: T, *args, **kwargs) -> T:
        """Reload ``sdaa`` from self.tensor."""
        return type(self)(self.tensor.sdaa(*args, **kwargs), clone=False)
