...
_bias_act_sdaa_cache: Dict = dict()


def _bias_act_sdaa(dim: int = 1,
                   act: str = 'linear',
                   alpha: Optional[Union[float, int]] = None,
                   gain: Optional[float] = None,
                   clamp: Optional[float] = None):
    """"Fast SDAA implementation of `bias_act()` using custom ops.
    ...
    if key in _bias_act_sdaa_cache:
        return _bias_act_sdaa_cache[key]
        ...
        def forward(ctx, x, b):  # pylint: disable=arguments-differ
            ...
            if act != 'linear' or gain != 1 or clamp >= 0 or (
                ...
                y = ext_module.bias_act(x, b, _null_tensor.to(x.device),
                                        ...
                                        spec.sdaa_idx, alpha, gain, clamp)
    ...
    class BiasActCudaGrad(torch.autograd.Function):
        ...
        def forward(ctx, dy, x, b, y):  # pylint: disable=arguments-differ
            ...
            dx = ext_module.bias_act(dy, b, x, y, _null_tensor.to(x.device), 1,
                                     dim, spec.sdaa_idx, alpha, gain, clamp)
            ctx.save_for_backward(
    ...
    # Add to cache.
    _bias_act_sdaa_cache[key] = BiasActCuda
    return BiasActCuda
