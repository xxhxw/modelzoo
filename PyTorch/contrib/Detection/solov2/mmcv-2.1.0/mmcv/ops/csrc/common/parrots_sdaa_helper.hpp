#ifndef PARROTS_SDAA_HELPER
#define PARROTS_SDAA_HELPER
...
#include "common_sdaa_helper.hpp"
#include "parrots_sdaawarpfunction.cuh"
...
#define PARROTS_SDAA_CHECK(exp)                         \
  do {                                                  \
    sdaaError_t err = exp;                              \
    if (err != sdaaSuccess) {                           \
      fprintf(stderr, "sdaaCheckError() failed : %s\n", \
              sdaaGetErrorString(err));                 \
      exit(-1);                                         \
    }                                                   \
  } while (0)
...
/** atomicAdd **/
#if defined(__SDAA_ARCH__) && __SDAA_ARCH__ < 600
