...
class TestSyncBN:
    ...
    def _test_syncbn_train(self, size=1, half=False):
        ...
        torch.manual_seed(9)
        torch.sdaa.manual_seed(9)

        self.x = torch.rand(16, 3, 2, 3).sdaa()
        self.y_bp = torch.rand(16, 3, 2, 3).sdaa()
    ...
    def _test_syncbn_empty_train(self, size=1, half=False):
        ...
        self.x = torch.rand(0, 3, 2, 3).sdaa()
        self.y_bp = torch.rand(0, 3, 2, 3).sdaa()
        ...
        syncbn = SyncBatchNorm(3, group=group, stats_mode='N').sdaa()
        syncbn.weight.data[0] = 0.2
        syncbn.weight.data[1] = 0.5
        syncbn.weight.data[2] = 0.7
        syncbn.train()
