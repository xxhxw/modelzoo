...
sdaa_devices = ['sdaa', 0, torch.device('sdaa')]
devices_to_test = cpu_devices
if torch.sdaa.is_available():
    devices_to_test += sdaa_devices
...
class TestBaseModel(TestCase):
    ...
    @unittest.skipIf(not torch.sdaa.is_available(), 'sdaa should be available')
    def test_sdaa(self):
        inputs = torch.randn(1, 3, 1, 1).sdaa()
        data = dict(inputs=inputs, data_sample=None)
        model = ToyModel().sdaa()
        out = model.val_step(data)
        self.assertEqual(out.device.type, 'sdaa')
        ...
        self.assertEqual(model.toy_model.data_preprocessor._device,
                         torch.device('cpu'))
        model.sdaa()
        self.assertEqual(model.data_preprocessor._device, torch.device('sdaa'))
        self.assertEqual(model.toy_model.data_preprocessor._device,
                         torch.device('sdaa'))

    @unittest.skipIf(not torch.sdaa.is_available(), 'sdaa should be available')
    def test_to(self):
        inputs = torch.randn(1, 3, 1, 1).to('sdaa:0')
        data = dict(inputs=inputs, data_sample=None)
        model = ToyModel().to(torch.sdaa.current_device())
        out = model.val_step(data)
        self.assertEqual(out.device.type, 'sdaa')
