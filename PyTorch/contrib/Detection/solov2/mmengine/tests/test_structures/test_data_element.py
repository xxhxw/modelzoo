...
class TestBaseDataElement(TestCase):
    ...
    @pytest.mark.skipif(
        not torch.sdaa.is_available(), reason='GPU is required!')
    def test_sdaa(self):
        metainfo, data = self.setup_data()
        instances = BaseDataElement(metainfo=metainfo, **data)

        sdaa_instances = instances.sdaa()
        self.check_data_device(sdaa_instances, 'sdaa:0')

        # here we further test to convert from sdaa to cpu
        cpu_instances = sdaa_instances.cpu()
        self.check_data_device(cpu_instances, 'cpu')
        del sdaa_instances

        sdaa_instances = instances.to('sdaa:0')
        self.check_data_device(sdaa_instances, 'sdaa:0')
