    ...
    try:
        ...
        if MUSA_AVAILABLE:
            ...
            if not torch.sdaa.is_available():
                return local_rank
            sdaa_visible_devices = os.getenv('SDAA_VISIBLE_DEVICES', None)
            if sdaa_visible_devices is None:
                num_device = torch.sdaa.device_count()
                sdaa_visible_devices = list(range(num_device))
            else:
                sdaa_visible_devices = sdaa_visible_devices.split(',')
            try:
                return int(sdaa_visible_devices[local_rank])
            except ValueError:
                # handle case for Multi-Instance GPUs
                # see #1148 for details
                return sdaa_visible_devices[local_rank]
