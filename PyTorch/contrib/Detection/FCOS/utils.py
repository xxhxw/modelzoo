...
def train_one_epoch(model, optimizer, scaler, data_loader, device, epoch, use_acm, rank, local_rank, img_size, lr, train_throughput, max_step, save_path):
    ...
    for step, data in enumerate(data_loader):
        ...
        batch_imgs=batch_imgs.sdaa()
        batch_boxes=batch_boxes.sdaa()
        batch_classes=batch_classes.sdaa()
        ...
        scaler.scale(loss.mean()).backward()    # loss缩放并反向传播
        ...
        if local_rank == 0 or local_rank == -1:
            with open(os.path.join(save_path, 'log.jsonl'), 'a') as f:
                json.dump({'Iteration': step, 'Loss': float(loss.mean().detach())}, f)
                f.write('\n')
    ...
    return accu_loss.item() / len(data_loader), total_data_to_device_time, compute_time, total_forward_time, total_backward_time, total_optimizer_step_time
