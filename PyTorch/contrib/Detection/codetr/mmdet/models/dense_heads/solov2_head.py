...
class SOLOV2Head(SOLOHead):
    ...
    def loss_by_feat(self, mlvl_kernel_preds: List[Tensor],
        ...
        for lvl_pos_kernel_preds in mlvl_pos_kernel_preds:
            ...
            for img_id, img_lvl_pos_kernel_pred in enumerate(
                ...
                # conv_result = F.conv2d(img_mask_feats, img_lvl_pos_kernel_pred.permute(1, 0).view(
                #         num_kernel, -1, self.dynamic_conv_size,
                #         self.dynamic_conv_size), stride=1).view(-1, h, w)
                # print(f'conv_result min: {conv_result.min()}, max: {conv_result.max()}')
    ...
    def _predict_by_feat_single(self,
        ...
        kernel_preds = kernel_preds.view(
            ...
            self.dynamic_conv_size)
        conv_result = F.conv2d(mask_feats, kernel_preds, stride=1).squeeze(0)
        # print(f'conv_result min: {conv_result.min()}, max: {conv_result.max()}')
        # print(cls_labels)
        mask_preds = F.conv2d(
