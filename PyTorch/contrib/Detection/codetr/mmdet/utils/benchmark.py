...
from mmengine.device import get_max_sdaa_memory
...
class InferenceBenchmark(BaseBenchmark):
    ...
    SDAA memory and CPU memory information.
        ...
        for i, data in enumerate(self.data_loader):
            ...
            if i >= self.num_warmup:
                ...
                if (i + 1) % self.log_interval == 0:
                    ...
                    sdaa_memory = get_max_sdaa_memory()

                    print_log(
                        f'Done image [{i + 1:<3}/{self.max_iter}], '
                        f'fps: {fps:.1f} img/s, '
                        f'times per image: {1000 / fps:.1f} ms/img, '
                        f'sdaa memory: {sdaa_memory} MB', self.logger)
                    print_process_memory(self._process, self.logger)
    ...
    def average_multiple_runs(self, results: List[dict]) -> dict:
        ...
        print_log(f'sdaa memory: {get_max_sdaa_memory()} MB', self.logger)
        print_process_memory(self._process, self.logger)
