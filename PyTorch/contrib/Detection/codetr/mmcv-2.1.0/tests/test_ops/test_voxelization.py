...
def test_voxelization_nondeterministic():
    ...
    points = torch.tensor(points).contiguous().to(device='sdaa:0')
    ...
    points = torch.tensor(points).contiguous().to(device='sdaa:0')[:max_voxels]
    coors_all = dynamic_voxelization.forward(points)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            # dipu will mock to sdaa automatically on mlu physical device.
            'sdaa:0',
            marks=pytest.mark.skipif(
                not IS_SDAA_AVAILABLE, reason='requires SDAA support')),
        pytest.param(
