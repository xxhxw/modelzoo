...
#include <fstream>
// 文件操作对象
std::ofstream outfile("/root/offset_input.txt");
...
void ROIAlignForward(const int nthreads, const T* input, const T* rois,
  ...
  // printf("nthreads: %d\n", nthreads);
  // printf("channels: %d\n", channels);
  // printf("pooled_width: %d\n", pooled_width);
  // printf("pooled_height: %d\n", pooled_height);
  ...
  for (int n = 0; n < n_rois; n++) {
    ...
    for (int c = 0; c < channels; c++) {
      ...
      // if(temp == 0){
      //   printf("output_val: %f\n", static_cast<float>(offset_input[sizeof(offset_input) / sizeof(offset_input[0]) - 1]));
      //   printf("roi_batch_ind: %f\n", static_cast<float>(roi_batch_ind));
      ...
      //   temp = 1;
      // }
      // 如果文件成功打开，将offset_input内容写入文件
        if (outfile.is_open()&&temp == 0) {
            outfile << "offset_input for channel " << c << ":\n";
            outfile << static_cast<float>(height) << static_cast<float>(width) << "\n";
            for (int i = 0; i < height * width; i++) {
              outfile << static_cast<float>(i) << "\n";
              outfile << static_cast<float>(offset_input[i]) << "\n";
            }
            temp = 1;
        }
      int pre_calc_index = 0;
