...
def points_in_boxes_part(points: Tensor, boxes: Tensor) -> Tensor:
    """Find the box in which each point is (SDAA).
    ...
    # will be created on the current device in the sdaa op,
    ...
    if torch.sdaa.current_device() != points_device:
        torch.sdaa.set_device(points_device)
...
def points_in_boxes_all(points: Tensor, boxes: Tensor) -> Tensor:
    """Find all boxes in which each point is (SDAA).
