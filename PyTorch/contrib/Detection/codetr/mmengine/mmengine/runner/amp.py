...
def autocast(device_type: Optional[str] = None,
    ...
    """A wrapper of ``torch.autocast`` and ``toch.sdaa.amp.autocast``.

    Pytorch 1.5.0 provide ``torch_sdaa.amp.autocast`` for running in
    ...
    ``torch.autocast`` and ``torch_sdaa.amp.autocast``, which resolves the
    compatibility issues that ``torch_sdaa.amp.autocast`` does not support
    ...
    Note:
        ...
        <= 1.10.0 and sdaa is not available, it will raise an error with
        ``enabled=True``, since ``torch_sdaa.amp.autocast`` only support sdaa
        mode.
    ...
    Examples:
         ...
         >>>    # raise error, torch_sdaa.amp.autocast only support sdaa mode.
         >>>    pass
         >>> # case2: Pytorch version >= 1.10.0
         >>> with autocast():
         >>>    # default sdaa mixed precision context
         >>>    pass
         >>> with autocast(device_type='cpu'):
         >>>    # cpu mixed precision context
         >>>    pass
         >>> with autocast(
         >>>     device_type='sdaa', enabled=True, cache_enabled=True):
         >>>    # enable precision context with more specific arguments.
         >>>    pass

    Args:
        device_type (str, required):  Whether to use 'sdaa' or 'cpu' device.
    ...
    if (digit_version('1.5.0') <= digit_version(TORCH_VERSION) <
        ...
        # dtype for `torch_sdaa.amp.autocast` is torch.float16.
        assert (
            device_type == 'sdaa' or device_type == 'mlu'
            or device_type is None), (
                'Pytorch version under 1.10.0 only supports running automatic '
                'mixed training with sdaa or mlu')
        ...
        if device_type == 'sdaa':
            ...
            if dtype == torch.bfloat16 and not \
                    torch.sdaa.is_bf16_supported():
                raise RuntimeError(
                    'Current SDAA Device does not support bfloat16. Please '
            ...
            if enabled is False:
                ...
                raise ValueError('User specified autocast device_type must be '
                                 f'sdaa or cpu, but got {device_type}')
