...
class BaseDataElement:
    ...
    Examples:
        ...
        >>> # Tensor-like
        >>> sdaa_instances = gt_instances.sdaa()
        >>> sdaa_instances = gt_instances.to('sdaa:0')
        >>> cpu_instances = sdaa_instances.cpu()
        >>> cpu_instances = sdaa_instances.to('cpu')
        >>> fp16_instances = sdaa_instances.to(
        ...     device=None, dtype=torch.float16, non_blocking=False,
        ...     copy=False, memory_format=torch.preserve_format)
        >>> cpu_instances = sdaa_instances.detach()
