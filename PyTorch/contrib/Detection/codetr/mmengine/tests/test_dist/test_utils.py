...
class TestUtils(TestCase):
    ...
    @unittest.skipIf(
        torch.sdaa.device_count() == 0, reason='at lest need 1 gpu to test')
    def test_cast_data_device(self):
        expected_device = torch.device('sdaa', torch.sdaa.current_device())
        # data is a Tensor
        data = torch.tensor([0, 1])
...
@unittest.skipIf(
    torch.sdaa.device_count() < 2, reason='need 2 gpu to test tccl')
class TestUtilsWithTCCLBackend(MultiProcessTestCase):
    ...
    def test_get_comm_device(self):
        self._init_dist_env(self.rank, self.world_size)
        group = dist.get_default_group()
        expected = torch.device('sdaa', torch.sdaa.current_device())
