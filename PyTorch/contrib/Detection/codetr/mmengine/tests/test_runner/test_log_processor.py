...
class TestLogProcessor(RunnerTestCase):
    ...
    def test_get_log_after_iter(self, by_epoch, mode, log_with_hierarchy):
        ...
        if by_epoch:
            ...
            if is_sdaa_available() or is_musa_available():
                log_str += 'memory: 100  '
            if mode == 'train':
    ...
    @patch('torch.sdaa.max_memory_allocated', MagicMock())
    @patch('torch.sdaa.reset_peak_memory_stats', MagicMock())
    def test_get_max_memory(self):
        ...
        torch.sdaa.max_memory_allocated.assert_called()
        torch.sdaa.reset_peak_memory_stats.assert_called()
