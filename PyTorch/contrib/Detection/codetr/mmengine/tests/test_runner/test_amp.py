    ...
    def test_autocast(self):
        if is_npu_available():
            ...
            if digit_version(TORCH_VERSION) < digit_version('1.10.0'):
                # `torch_sdaa.amp.autocast` is only support in gpu mode, if
                # sdaa is not available, it will return an empty context and
            ...
            for device in devices:
                with autocast(device_type=device):
                    # torch.autocast support cpu and sdaa mode.
                    layer = nn.Conv2d(1, 1, 1).to(device)
                    res = layer(torch.randn(1, 1, 1, 1).to(device))
                    self.assertIn(res.dtype, (torch.bfloat16, torch.float16))
                    with autocast(enabled=False, device_type=device):
