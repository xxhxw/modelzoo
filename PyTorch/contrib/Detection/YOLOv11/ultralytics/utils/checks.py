...
def collect_system_info():
    ...
    gib = 1 << 30  # bytes per GiB
    sdaa = torch and torch.sdaa.is_available()
    check_yolo()
    total, used, free = shutil.disk_usage("/")
    ...
    info_dict = {
        ...
        "GPU": get_gpu_info(index=0) if sdaa else None,
        "GPU count": torch.sdaa.device_count() if sdaa else None,
        "SDAA": torch.version.sdaa if sdaa else None,
    }
...
def check_amp(model):
    ...
    Example:
        ...
        model = YOLO("yolo11n.pt").model.sdaa()
        check_amp(model)
        ```
    ...
    if device.type in {"cpu", "mps"}:
        ...
        gpu = torch.sdaa.get_device_name(device)
