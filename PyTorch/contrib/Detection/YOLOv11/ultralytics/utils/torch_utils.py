...
@contextmanager
def sdaa_memory_usage(device=None):
    """
    ...
    Args:
        device (torch.device, optional): The SDAA device to query memory usage for. Defaults to None.

    Yields:
        (dict): A dictionary with a key 'memory' initialized to 0, which will be updated with the reserved memory.
    """
    sdaa_info = dict(memory=0)
    if torch.sdaa.is_available():
        torch.sdaa.empty_cache()
        try:
            yield sdaa_info
        finally:
            sdaa_info["memory"] = torch.sdaa.memory_reserved(device)
    else:
        yield sdaa_info
    ...
    for x in input if isinstance(input, list) else [input]:
        ...
        for m in ops if isinstance(ops, list) else [ops]:
            ...
            try:
                mem = 0
                for _ in range(n):
                    with sdaa_memory_usage(device) as sdaa_info:
                        t[0] = time_sync()
                        y = m(x)
                        t[1] = time_sync()
                        try:
