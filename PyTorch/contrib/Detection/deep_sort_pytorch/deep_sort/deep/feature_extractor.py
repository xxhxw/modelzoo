...
class Extractor(object):
    def __init__(self, model_path, use_sdaa=True):
        self.net = Net(reid=True)
        # self.net = resnet18(reid=True)
        # self.device = "cuda" if torch.cuda.is_available() and use_cuda else "cpu"
        self.device = "sdaa" if torch.sdaa.is_available() and use_sdaa else "cpu"
...
class FastReIDExtractor(object):
    def __init__(self, model_config, model_path, use_sdaa=True):
        cfg = get_cfg()
