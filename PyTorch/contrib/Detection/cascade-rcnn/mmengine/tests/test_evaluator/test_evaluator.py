...
class TestEvaluator(TestCase):
    ...
    @unittest.skipUnless(torch.sdaa.is_available(), 'can only run with gpu')
    def test_evaluate_cast_cpu(self):
        ...
        all_data = [
            dict(
                inputs=torch.zeros((3, 10, 10), device='sdaa'),
                data_sample=BaseDataElement(
                    label=torch.ones((1, ), device='sdaa')))
            for _ in range(size)
        ]
        all_predictions = [
            BaseDataElement(
                pred=torch.zeros((1, ), device='sdaa'),
                label=torch.ones((1, ), device='sdaa')) for _ in range(size)
        ]
