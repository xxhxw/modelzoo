...
class TestBaseDataPreprocessor(TestCase):
    ...
    def test_forward(self):
        ...
        if torch.sdaa.is_available():
            ...
            data = dict(inputs=[input1, input2], data_sample=[label1, label2])
            base_data_preprocessor = base_data_preprocessor.sdaa()
            output = base_data_preprocessor(data)
            batch_inputs, batch_labels = output['inputs'], output[
                'data_sample']
            self.assertTrue(torch.is_floating_point(batch_inputs[0]))
            self.assertEqual(batch_inputs[0].device.type, 'sdaa')
            ...
            # Test `base_data_preprocessor` can be moved to sdaa again.
            base_data_preprocessor = base_data_preprocessor.to('sdaa:0')
            output = base_data_preprocessor(data)
            batch_inputs, batch_labels = output['inputs'], output[
                'data_sample']
            self.assertTrue(torch.is_floating_point(batch_inputs[0]))
            ...
            # device of `base_data_preprocessor` is sdaa, output should be
            # sdaa tensor.
            self.assertEqual(batch_inputs[0].device.type, 'sdaa')
            self.assertEqual(batch_labels[0].device.type, 'sdaa')
