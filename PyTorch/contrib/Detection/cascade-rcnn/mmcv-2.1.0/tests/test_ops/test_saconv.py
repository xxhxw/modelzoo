...
def test_sacconv():
    ...
    if torch.sdaa.is_available():
        x = torch.rand(1, 3, 256, 256).sdaa()
        deform_saconv = SAConv2d(
            3, 5, kernel_size=3, padding=1, use_deform=True).sdaa()
        deform_sac_out = deform_saconv(x).sdaa()
        refer_conv = nn.Conv2d(3, 5, kernel_size=3, padding=1).sdaa()
        refer_out = refer_conv(x)
