...
def main():
    ...
    #修改后
    args.sdaa = not args.no_cuda and torch.sdaa.is_available()
    print(args)
    ...
    val_loader = torch.utils.data.DataLoader(
        ...
        #修改后
        num_workers=args.workers, pin_memory=True if args.sdaa else False)
    ...
    if args.sdaa:
        model.sdaa()
        # Please use sdaa_VISIBLE_DEVICES to control the number of gpus
        model = nn.DataParallel(model)
    ...
    for batch_idx, (data, target) in enumerate(tbar):
        ...
        #修改后
        if args.sdaa:
            data, target = data.sdaa(), target.sdaa()
        with torch.no_grad():
            output = model(data)
