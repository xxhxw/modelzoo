...
class Engine(object):
    def __init__(self, local_rank, for_val_only=False, base_config=None):
        ...
        if num_gpus > 1 and not for_val_only:
            ...
            torch.sdaa.set_device(self.local_rank)
            
            
            #修改dist.init_process_group中backend从'nccl'到'tccl'
    ...
    #修改set_value函数，将参数从cuda上转移到sdaa设备上
    ...
    def set_value(self, param, value):
        ...
        #     print('not equal: ', value.size, param.nelement)
        #     assert 0
        param.data = torch.from_numpy(value).sdaa().type(torch.sdaa.FloatTensor)
