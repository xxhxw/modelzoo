...
from data.data_factory import create_dataset, load_sdaa_data, num_iters_per_epoch
...
def train_one_step(net, data, label, optimizer, criterion,
    ...
    # 每 100 次迭代绘制并保存全局平均 loss 曲线
    if len(loss_history) % 100 == 0:
        iterations = list(range(1, len(loss_history) + 1))
        plt.figure()
        plt.plot(iterations, loss_history, label="Loss", color="blue", linewidth=2)
        ...
        plt.savefig('./scripts/train_sdaa_3rd_ACNet.png')
...
save_hdf5_epochs=10000):
    ...
    with Engine(local_rank=local_rank) as engine:
        ...
        engine.echo('NOTE: We have global_batch_size={} on {} GPUs, the allocated GPU memory is {}'.format(cfg.global_batch_size, torch.sdaa.device_count(), torch.sdaa.memory_allocated()))
        # ----------------------------- data done --------------------------------
        ...
        criterion = get_criterion(cfg).sdaa()
        # --------------------------------- done -------------------------------
        ...
        if engine.distributed:
            ...
            assert torch.sdaa.device_count() == 1
        ...
        if gradient_mask is not None:
            ...
            for name, value in gradient_mask.items():
                #修改gradient_mask_tensor[name]到sdaa设备上
                #gradient_mask_tensor[name] = torch.Tensor(value).cuda()
                gradient_mask_tensor[name] = torch.Tensor(value).sdaa()
        else:
        ...
        for epoch in range(done_epochs, cfg.max_epochs):
            ...
            for _ in pbar:
                ...
                data, label = load_sdaa_data(train_data, dataset_name=cfg.dataset_name)
