...
#     return data, label
#将data数据送到sdaa设备上
def load_sdaa_data(data_loader, dataset_name):
    if dataset_name == 'imagenet_standard':
        data, label = next(data_loader.dataprovider)
        data = data.sdaa()
        label = label.sdaa()
...
class ImageNetBlankGenerator(object):
    ...
    def __init__(self, batch_size, img_size):
        ...
        self.return_dict = {'data': torch.from_numpy(self.blank_img).type(torch.FloatTensor).sdaa(),
                            'label': torch.from_numpy(self.blank_label).type(torch.long).sdaa()}
...
def create_dataset(dataset_name, subset, global_batch_size, distributed):
    ...
    if dataset_name == 'imagenet_standard':
        ...
        if subset == 'train':
            ...
            return ImgnetStdTrainData(distributed=distributed,
                                      batch_size_per_gpu=global_batch_size // torch.sdaa.device_count())
        else:
