...
class MoCo_Model(nn.Module):
    ...
    def shuffled_idx(self, batch_size):
        ...
        shuffled_idxs = torch.randperm(batch_size).long().sdaa()

        reverse_idxs = torch.zeros(batch_size).long().sdaa()

        value = torch.arange(batch_size).long().sdaa()
    ...
    def InfoNCE_logits(self, f_q, f_k):
        ...
        labels = torch.zeros(logits.shape[0], dtype=torch.long).sdaa()
