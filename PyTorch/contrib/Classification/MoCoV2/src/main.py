...
def setup(distributed):
    ...
    if distributed:
        ...
        device = torch.device(f'sdaa:{local_rank}')  # unique on individual node
        ...
        local_rank = None
        device = torch.device('sdaa:0' if torch.sdaa.is_available() else 'cpu')
...
def main():
    ...
    if args.distributed:
        ...
        print('\nUsing', torch.sdaa.device_count(), 'GPU(s).\n')
