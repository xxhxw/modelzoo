...
@register_model
@register_model
def EvoLeViT_256_384(
    num_classes=1000, 
    distillation=True,
    pretrained=False, 
    fuse=False,
    pretrained_cfg=None,       # 新增
    pretrained_cfg_overlay=None,  # 新增
    cache_dir=None,            # 新增
    **kwargs                   # 捕获其他参数
):
    return model_factory(
        **specification['EvoLeViT_256_384'], 
...
class LeViT(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.flatten(2).transpose(1, 2)  # 这里可能使 x 不连续
        ...
        cls = x[:, 0, :].contiguous()  # 关键修复：确保 cls 是连续的
        x = x[:, 1:, :]
        x = x.mean(1)
