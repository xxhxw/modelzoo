...
def parse_option():
    ...
    parser.add_argument('--data-path', default='/data/datasets/imagenet', type=str, help='path to dataset')
    ...
    parser.add_argument('--output', default='/data/ckpt/', type=str, metavar='PATH',
...
def main(config):
    ...
    model = get_RepMLPNet_model(config.MODEL.ARCH, deploy=False)

    optimizer = build_optimizer(config, model)

    model.to("sdaa")

    if torch.sdaa.device_count() > 1:
        cur_device = torch.sdaa.current_device()
        if config.AMP_OPT_LEVEL != "O0":
            model, optimizer = amp.initialize(model, optimizer, opt_level=config.AMP_OPT_LEVEL)
        # model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[config.LOCAL_RANK],
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[cur_device],
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, model_ema=None):
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        samples = samples.sdaa(non_blocking=True)
        targets = targets.sdaa(non_blocking=True)
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            memory_used = torch.sdaa.max_memory_allocated() / (1024.0 * 1024.0)
        ...
        if (end - start) >= 7200:
            logger.info(f"Reach Limited Time(2 hours)!")
            break
...
def validate(config, data_loader, model):
    ...
    for idx, (images, target) in enumerate(data_loader):
        images = images.sdaa(non_blocking=True)
        target = target.sdaa(non_blocking=True)
...
if __name__ == '__main__':
    ...
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        rank = -1
        world_size = -1
    # torch.sdaa.set_device(config.LOCAL_RANK)
    torch.sdaa.set_device(rank)
    # torch.distributed.init_process_group(backend='nccl', init_method='env://', world_size=world_size, rank=rank)
    torch.distributed.init_process_group(backend='tccl', init_method='env://', world_size=world_size, rank=rank)
    ...
    logger = create_logger(output_dir=config.OUTPUT, dist_rank=0 if torch.sdaa.device_count() == 1 else dist.get_rank(), name=f"{config.MODEL.ARCH}")

    if torch.sdaa.device_count() == 1 or dist.get_rank() == 0:
