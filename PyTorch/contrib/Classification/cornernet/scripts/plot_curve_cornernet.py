...
directory = '../work_dirs/cornernet_hourglass104_8xb6-210e-mstest_coco'
latest_folder = get_latest_folder(directory)

print(f"The latest folder is: {latest_folder}")
latest_file = '../work_dirs/cornernet_hourglass104_8xb6-210e-mstest_coco/' + latest_folder + '/vis_data/scalars.json'
...
def plot_loss_from_file(file_path):
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            off_loss_values.append(data['off_loss'])
            pull_loss_values.append(data['pull_loss'])
            push_loss_values.append(data['push_loss'])

    # 创建output文件夹（如果不存在）
    output_dir = 'output_cornernet'
    ...
    loss_data = [
        ...
        ('Off Loss', off_loss_values, 'red'),
        ('Pull Loss', pull_loss_values, 'green'),
        ('Push Loss', push_loss_values, 'purple')
    ]
