...
# 加载模型
from main import GoogLeNet
# from Swin_Transformer.models.swin_transformer_acmix import SwinTransformer_acmix # type: ignore
# local_rank = int(os.environ.get("LOCAL_RANK", -1))
# import sys
# sys.path.append('/data/bupt/ACmix/Swin_Transformer')
# # 加载模型
# from Swin_Transformer.models.swin_transformer_acmix import SwinTransformer_acmix
torch.cuda.empty_cache()  # 释放显存
torch.set_num_threads(4)  # 限制 CPU 计算线程
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        if args.local_rank == 0:
            ...
            with open(f'./experiments/training_log_batchsize_{args.batch_size}_lr_{args.str_lr}.txt', 'a') as f:
                f.write(f"Epoch {epoch}, Train Loss: {train_loss:.4f}, Train Acc: {train_acc:.4f}, "f"Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.4f}\n")
    ...
    if args.local_rank == 0 and args.step < 0:
        ...
        plt.savefig(f'./experiments/loss_curve_batchsize_{args.batch_size}_lr_{args.str_lr}.png')
        ...
        plt.savefig(f'./experiments/accuracy_curve_batchsize_{args.batch_size}_lr_{args.str_lr}.png')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_path', type=str,
                        default="/root/data/bupt/datasets/imagenet")
    ...
    json_logger = Logger(
        [
            ...
            JSONStreamBackend(Verbosity.VERBOSE, os.path.join(opt.path,  f"log_batchsize_{opt.batch_size}_lr_{opt.str_lr}.json")),
        ]
    )
    json_logger.metadata("train.loss", {"unit": "", "GOAL": "MINIMIZE", "STAGE": "TRAIN"})
    json_logger.metadata("train.acc", {"unit": "", "GOAL": "MAXIMIZE", "STAGE": "TRAIN"})
