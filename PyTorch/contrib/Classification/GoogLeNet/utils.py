...
def get_datasets(dataset_path):
    full_train_dataset = ImageFolder(os.path.join(dataset_path, 'train'))
    full_val_dataset = ImageFolder(os.path.join(dataset_path, 'val'))

    # 获取所有类别
    class_names = full_train_dataset.classes  # ImageFolder 自动获取类别
    print(f"原始类别数量: {len(class_names)}")

    # 选择前 20 个类别
    selected_classes = class_names[:20]
    print(f"使用类别: {selected_classes}")

    # 过滤出只包含前 20 类的样本
    train_samples = [sample for sample in full_train_dataset.samples if sample[1] < 20]
    val_samples = [sample for sample in full_val_dataset.samples if sample[1] < 20]

    # 重新创建子数据集
    train_dataset = ImageFolder(os.path.join(dataset_path, 'train'), transform=train_transforms)
    train_dataset.samples = train_samples
    train_dataset.targets = [label for _, label in train_samples]

    val_dataset = ImageFolder(os.path.join(dataset_path, 'val'), transform=val_transforms)
    val_dataset.samples = val_samples
    val_dataset.targets = [label for _, label in val_samples]

    print(f"新训练集大小: {len(train_dataset)}，新验证集大小: {len(val_dataset)}")
...
def train_one_epoch(model, optimizer, scaler, data_loader, device, epoch, use_acm, rank, json_logger, img_size, lr, train_throughput,max_step):
    ...
    sample_num = 0
    data_loader = tqdm(data_loader, file=sys.stdout,mininterval=45)
