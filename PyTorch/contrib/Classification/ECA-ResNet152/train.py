...
# 加载模型
from eca_resnet import eca_resnet152
...
def main(args):
    ...
    # 如果存在预训练权重则载入
    model = eca_resnet152(num_classes=1000)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', type=str, default='/data/ckpt/ECA-ResNet152/experiments/')

    opt = parser.parse_args()
    
    opt.str_lr = str(opt.lr).replace('.', '_')
    if not opt.distributed:
        opt.path = '/data/ckpt/ECA-ResNet152/single_experiments/'
