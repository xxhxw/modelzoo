...
def main(args):
    ...
    # 如果存在预训练权重则载入
    model = WaveMLP_T(num_classes=1000)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', type=str, default='/data/ckpt/Wave-MLP/experiments/')

    opt = parser.parse_args()
    
    opt.str_lr = str(opt.lr).replace('.', '_')
    if not opt.distributed:
        opt.path = '/data/ckpt/Wave-MLP/single_experiments/'
