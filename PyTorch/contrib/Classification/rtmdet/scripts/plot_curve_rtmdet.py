...
directory = '../work_dirs/rtmdet_s_8xb32-300e_coco'
latest_folder = get_latest_folder(directory)

print(f"The latest folder is: {latest_folder}")
latest_file = '../work_dirs/rtmdet_s_8xb32-300e_coco/' + latest_folder + '/vis_data/scalars.json'
...
def plot_loss_from_file(file_path):
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            off_loss_values.append(data['loss_cls'])
            pull_loss_values.append(data['loss_bbox'])
            # push_loss_values.append(data['push_loss'])

    # 创建output文件夹（如果不存在）
    output_dir = 'output_rtmdet'
    ...
    loss_data = [
        ...
        ('loss_cls', off_loss_values, 'red'),
        ('loss_bbox', pull_loss_values, 'green'),
