...
import torch
import torch_sdaa  # [修改] 导入torch_sdaa模块
import torch.nn as nn
import torch.nn.parallel
# [修改] 移除cudnn，在TecoPytorch中不支持
...
parser.add_argument('--dist-backend', default='tccl', type=str,  # [修改] 默认后端改为tccl
...
def main():
    global args, best_err1, best_err5, device  # [修改] 添加全局device变量
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            checkpoint = torch.load(args.resume, map_location=device)  # [修改] 添加map_location
    ...
    # [修改] 不使用cudnn.benchmark
    # cudnn.benchmark = True

    if args.evaluate:
        validate(val_loader, model, criterion, 0)  # [修改] 添加epoch参数
        return
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # [修改] 将数据和标签放到SDAA设备上，并转换为channels_last格式
...
def validate(val_loader, model, criterion, epoch):
    ...
    with torch.no_grad():  # [修改] 使用no_grad()替代volatile=True
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    directory = "/data/ckpt/PyramidNet_pretrain/%s/" % (args.expname)
    if not os.path.exists(directory):
        os.makedirs(directory)
    filename = directory + filename
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, '/data/ckpt/PyramidNet_pretrain/%s/' % (args.expname) + 'model_best.pth.tar')
...
def accuracy(output, target, topk=(1,)):
    ...
    res = []
    for k in topk:
        # [修改] 使用reshape代替view，避免在PyTorch 2.4中的兼容性问题
