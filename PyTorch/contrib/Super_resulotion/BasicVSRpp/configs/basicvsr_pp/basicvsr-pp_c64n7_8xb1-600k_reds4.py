...
train_dataloader = dict(
    num_workers=6, batch_size=1, dataset=dict(num_input_frames=15))
...
optim_wrapper = dict(
    constructor='DefaultOptimWrapperConstructor',
    type='OptimWrapper',
    optimizer=dict(type='SGD', lr=2e-4),
    paramwise_cfg=dict(custom_keys={'spynet': dict(lr_mult=0.125)}))

default_hooks = dict(checkpoint=dict(out_dir=save_dir),logger=dict(type='LoggerHook', interval=10))
