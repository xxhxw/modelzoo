...
def _demo_input_train(img_shape, batch_size=1, sdaa=False, meta={}):
...
def _demo_input_test(img_shape, batch_size=1, sdaa=False, meta={}):
    """Create a superset of inputs needed to run backbone.
...
def test_dim():
    ...
    if torch.sdaa.is_available():
        model = MODELS.build(model_cfg)
        model.sdaa()
        input_train = _demo_input_train((64, 64), sdaa=True)
        output_train = model(*input_train, mode='loss')
        assert_dict_keys_equal(output_train,
                               ['loss_alpha', 'loss_comp', 'loss_refine'])
    ...
    with torch.no_grad():
        ...
        if torch.sdaa.is_available():
            model = MODELS.build(model_cfg)
            model.sdaa()
            input_test = _demo_input_test((48, 48), sdaa=True)
            output_test = model(*input_test, mode='predict')
...
def test_indexnet():
    ...
    with torch.no_grad():
        ...
        if torch.sdaa.is_available():
            indexnet = MODELS.build(model_cfg).sdaa()
            indexnet.eval()
            input_test = _demo_input_test((48, 48), sdaa=True)
            output_test = indexnet(*input_test, mode='predict')
        assert_pred_alpha(output_test, (48, 48))
    ...
    if torch.sdaa.is_available():
        ...
        indexnet = MODELS.build(model_cfg).sdaa()
        input_train = _demo_input_train((64, 64), batch_size=2, sdaa=True)
        output_train = indexnet(*input_train, mode='loss')
        # assert output_train['num_samples'] == 2
...
def test_gca():
    ...
    with torch.no_grad():
        ...
        if torch.sdaa.is_available():
            model = MODELS.build(model_cfg)
            model.sdaa()
            inputs = _demo_input_test((48, 48), sdaa=True, meta=meta)
            outputs = model(*inputs, mode='predict')
            assert_pred_alpha(outputs, (48, 48))
