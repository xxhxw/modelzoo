...
class GANLoss(nn.Module):
    ...
    def __init__(self, d_last_layer_size):
        ...
        self.label_tensor_fake = Variable(torch.zeros(d_last_layer_shape).sdaa(), requires_grad=False)
        self.label_tensor_real = Variable(torch.ones(d_last_layer_shape).sdaa(), requires_grad=False)
...
class DownScaleLoss(nn.Module):
    ...
    def __init__(self, scale_factor):
        ...
        self.bicubic_kernel = Variable(torch.Tensor(bicubic_k).sdaa(), requires_grad=False)
...
class CentralizedLoss(nn.Module):
    ...
    def __init__(self, k_size, scale_factor=.5):
        ...
        self.indices = Variable(torch.arange(0., float(k_size)).sdaa(), requires_grad=False)
        wanted_center_of_mass = k_size // 2 + 0.5 * (int(1 / scale_factor) - k_size % 2)
        self.center = Variable(torch.FloatTensor([wanted_center_of_mass, wanted_center_of_mass]).sdaa(), requires_grad=False)
        self.loss = nn.MSELoss()
...
class BoundariesLoss(nn.Module):
    ...
    def __init__(self, k_size):
        ...
        self.zero_label = Variable(torch.zeros(k_size).sdaa(), requires_grad=False)
        self.loss = nn.L1Loss()
