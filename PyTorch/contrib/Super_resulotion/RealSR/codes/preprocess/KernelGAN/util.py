...
def im2tensor(im_np):
    ...
    return torch.FloatTensor(np.transpose(im_np, (2, 0, 1)) * 2.0 - 1.0).unsqueeze(0).sdaa()


def map2tensor(gray_map):
    """Move gray maps to GPU, no normalization is done"""
    return torch.FloatTensor(gray_map).unsqueeze(0).unsqueeze(0).sdaa()
...
def create_gaussian(size, sigma1, sigma2=-1, is_tensor=False):
    ...
    return torch.FloatTensor(np.outer(func1, func2)).sdaa() if is_tensor else np.outer(func1, func2)
