...
class SRGANModel(BaseModel):
    def __init__(self, opt):
        ...
        if opt['dist']:
            # self.netG = DistributedDataParallel(self.netG, device_ids=[torch.sdaa.current_device()])
        ...
        if self.is_train:
            ...
            if opt['dist']:
                ...
                #                                     device_ids=[torch.sdaa.current_device()])
    ...
    #             mid1 = torch.mean(pred_g_fake)
    #             mid2 = torch.mean(pred_d_real)
    #             l_g_gan = self.l_gan_w * (
    #                 self.cri_gan(pred_d_real - mid1, False) +
    #                 self.cri_gan(pred_g_fake - mid2, True)) / 2
    ...
    def back_projection(self):
        ...
        us_error = torch.nn.functional.interpolate(lr_error,
                                                   ...
                                                   mode='bicubic',
                                                   align_corners=False)
        self.fake_H = self.fake_H+self.opt['back_projection_lamda'] * us_error
    ...
    def forward_chop(self, *args, shave=10, min_size=160000):
        ...
        n_GPUs = min(torch.sdaa.device_count(), 4)
