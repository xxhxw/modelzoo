...
def test_perceptual_loss(init_weights):
    if torch.sdaa.is_available():
        loss_percep = PerceptualLoss(layer_weights={'0': 1.}).sdaa()
        x = torch.randn(1, 3, 16, 16).sdaa()
        x.requires_grad = True
        gt = torch.randn(1, 3, 16, 16).sdaa()
        ...
        loss_percep = PerceptualLoss(
            layer_weights={
                '0': 1.
            }, style_weight=0., criterion='mse').sdaa()
        x = torch.randn(1, 3, 16, 16).sdaa()
        gt = torch.randn(1, 3, 16, 16).sdaa()
        percep, style = loss_percep(x, gt)
...
def test_t_perceptual_loss():
    ...
    if torch.sdaa.is_available():
        maps = [
            torch.rand((2, 8, 8, 8)).sdaa(),
            torch.rand((2, 4, 16, 16)).sdaa()
        ]
        textures = [
            torch.rand((2, 8, 8, 8)).sdaa(),
            torch.rand((2, 4, 16, 16)).sdaa()
        ]
        soft = torch.rand((2, 1, 8, 8)).sdaa()
        loss_t_percep = TransferalPerceptualLoss().sdaa()
        ...
        loss_t_percep = TransferalPerceptualLoss(
            use_attention=False, criterion='l1').sdaa()
        t_percep = loss_t_percep(maps, soft, textures)
        assert t_percep.item() > 0
