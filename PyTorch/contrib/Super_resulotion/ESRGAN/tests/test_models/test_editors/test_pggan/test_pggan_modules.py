...
class TestEqualizedLR:
    ...
    def test_equalized_conv_module_cuda(self):
        conv = EqualizedLRConvModule(**self.default_conv_cfg).sdaa()
        res = conv(self.conv_input.sdaa())
        assert res.shape == (2, 1, 5, 5)
        has_equalized_lr = False
        for _, v in conv.conv._forward_pre_hooks.items():
            if isinstance(v, EqualizedLR):
    ...
    def test_equalized_linear_module_cuda(self):
        linear = EqualizedLRLinearModule(2, 2).sdaa()
        res = linear(self.linear_input.sdaa())
        assert res.shape == (2, 2)
        has_equalized_lr = False
        for _, v in linear._forward_pre_hooks.items():
            if isinstance(v, EqualizedLR):
...
class TestEqualizedLRConvUpModule:
    ...
    def test_equalized_lr_convup_module_cuda(self):
        convup = EqualizedLRConvUpModule(**self.default_cfg).sdaa()

        res = convup(self.default_input.sdaa())
        assert res.shape == (2, 1, 10, 10)
        # test bp
        res = convup(torch.randn((2, 3, 5, 5), requires_grad=True).sdaa())
        assert res.shape == (2, 1, 10, 10)
        res.mean().backward()
...
class TestEqualizedLRConvDownModule:
    ...
    def test_equalized_lr_conv_down_cuda(self):
        convdown = EqualizedLRConvDownModule(**self.default_cfg).sdaa()
        res = convdown(self.default_input.sdaa())
        assert res.shape == (2, 1, 4, 4)
        # test bp
        res = convdown(torch.randn((2, 3, 8, 8), requires_grad=True).sdaa())
        assert res.shape == (2, 1, 4, 4)
        res.mean().backward()
...
class TestPixelNorm:
    ...
    def test_pixel_norm_cuda(self):
        pn = PixelNorm().sdaa()
        res = pn(self.input_tensor.sdaa())
        assert res.shape == (2, 3, 4, 4)

        # test zero case
        res = pn(self.input_tensor.sdaa() * 0)
        assert res.shape == (2, 3, 4, 4)
        assert (res == 0).all()
...
class TestMiniBatchStddevLayer:
    ...
    def test_minibatch_stddev_layer_cuda(self):
        ministd_layer = MiniBatchStddevLayer().sdaa()
        res = ministd_layer(self.default_input.sdaa())
        assert res.shape == (2, 4, 4, 4)

        ministd_layer = MiniBatchStddevLayer(group_size=3).sdaa()
        res = ministd_layer(torch.randn((2, 6, 4, 4)).sdaa())
        assert res.shape == (2, 7, 4, 4)

        # test bp
        ministd_layer = MiniBatchStddevLayer().sdaa()
        res = ministd_layer(self.default_input.requires_grad_().sdaa())
        assert res.shape == (2, 4, 4, 4)
        res.mean().backward()
...
class TestPGGANNoiseTo2DFeat:
    ...
    def test_pggan_noise2feat_cuda(self):
        module = PGGANNoiseTo2DFeat(**self.default_cfg).sdaa()
        res = module(self.default_input.sdaa())
        assert res.shape == (2, 1, 4, 4)
        assert isinstance(module.linear, EqualizedLRLinearModule)
        assert not module.linear.bias
        assert module.with_norm
