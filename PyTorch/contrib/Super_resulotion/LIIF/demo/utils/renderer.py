...
class Renderer:
    ...
    def __init__(self, disable_timing=False):
        self._device = torch.device('sdaa' if torch.sdaa.is_available(
        ...
        if not disable_timing:
            self._start_event = torch.sdaa.Event(enable_timing=True)
            self._end_event = torch.sdaa.Event(enable_timing=True)
    ...
    def render(self, **args):
        if self._disable_timing:
            ...
            self._start_event.record(torch.sdaa.current_stream(self._device))
        ...
        if not self._disable_timing:
            self._end_event.record(torch.sdaa.current_stream(self._device))
        if 'image' in res:
