...
class TestDownsampleUpFIRDn():
    ...
    def test_DownsampleUpFIRDn_cuda(self):
        downsample = DownsampleUpFIRDn((2, 2), 2).sdaa()
        assert downsample.pad == (0, 0)

        inp = torch.randn(1, 3, 4, 4).sdaa()
        out = downsample(inp)
        assert out.shape == (1, 3, 2, 2)
...
class TestModStyleConv:
    ...
    def test_mod_styleconv_cuda(self):
        conv = ModulatedStyleConv(**self.default_cfg).sdaa()
        input_x = torch.randn((2, 3, 4, 4)).sdaa()
        input_style = torch.randn((2, 5)).sdaa()
        ...
        _cfg = deepcopy(self.default_cfg)
        _cfg['upsample'] = False
        conv = ModulatedStyleConv(**_cfg).sdaa()
        input_x = torch.randn((2, 3, 4, 4)).sdaa()
        input_style = torch.randn((2, 5)).sdaa()
...
class TestModulatedConv2d():
    ...
    def test_mod_conv_cuda(self):
        conv = ModulatedConv2d(**self.default_cfg).sdaa()
        input_x = torch.randn((2, 3, 4, 4)).sdaa()
        input_style = torch.randn((2, 5)).sdaa()
        ...
        _cfg = deepcopy(self.default_cfg)
        _cfg['upsample'] = False
        conv = ModulatedConv2d(**_cfg).sdaa()
        input_x = torch.randn((2, 3, 4, 4)).sdaa()
        input_style = torch.randn((2, 5)).sdaa()
        ...
        _cfg['downsample'] = True
        conv = ModulatedConv2d(**_cfg).sdaa()
        input_x = torch.randn((2, 3, 8, 8)).sdaa()
        input_style = torch.randn((2, 5)).sdaa()
        res = conv(input_x, input_style)
        assert res.shape == (2, 1, 4, 4)

        # test input gain
        res = conv(input_x, input_style, input_gain=torch.randn(2, 3).sdaa())
        assert res.shape == (2, 1, 4, 4)
...
class TestToRGB:
    ...
    def test_torgb_cuda(self):
        model = ModulatedToRGB(**self.default_cfg).sdaa()
        input_x = torch.randn((2, 5, 4, 4)).sdaa()
        style = torch.randn((2, 5)).sdaa()

        res = model(input_x, style)
        assert res.shape == (2, 3, 4, 4)

        input_x = torch.randn((2, 5, 8, 8)).sdaa()
        style = torch.randn((2, 5)).sdaa()
        skip = torch.randn(2, 3, 4, 4).sdaa()
        res = model(input_x, style, skip)
        assert res.shape == (2, 3, 8, 8)
