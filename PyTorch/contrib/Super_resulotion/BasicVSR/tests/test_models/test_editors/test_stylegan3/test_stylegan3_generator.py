...
class TestStyleGAN3Generator:
    ...
    def test_cuda(self):
        generator = StyleGAN3Generator(**self.default_cfg).sdaa()
        z = torch.randn((2, 6)).sdaa()
        c = None
        y = generator(z, c)
        assert y.shape == (2, 3, 16, 16)
        ...
        cfg = deepcopy(self.default_cfg)
        cfg.update(dict(rgb2bgr=True))
        generator = StyleGAN3Generator(**cfg).sdaa()
        y = generator(None, num_batches=2)
        assert y.shape == (2, 3, 16, 16)

        generator = StyleGAN3Generator(**self.s3_r_cfg).sdaa()
        z = torch.randn((2, 6)).sdaa()
        c = None
        y = generator(z, c)
        assert y.shape == (2, 3, 16, 16)
