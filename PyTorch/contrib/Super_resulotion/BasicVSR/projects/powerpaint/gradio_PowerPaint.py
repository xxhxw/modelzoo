...
depth_estimator = DPTForDepthEstimation.from_pretrained(
    'Intel/dpt-hybrid-midas').to('sdaa')
...
def get_depth_map(image):
    image = feature_extractor(
        images=image, return_tensors='pt').pixel_values.to('sdaa')
    with torch.no_grad(), torch.autocast('sdaa'):
...
def predict_controlnet(input_image, input_control_image, control_type, prompt,
    ...
    control_pipe = control_pipe.to('sdaa')
    current_control = 'canny'
