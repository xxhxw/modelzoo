...
class MIR1K(Dataset):
    def __init__(self, path, hop_length, groups=None, whole_audio=False, use_aug=True, rank=None):
        self.path = path
        ...
        if not rank:
            ...
            for group in groups:
                for input_files in self.files(group):
                    self.load(*input_files)
    ...
    def load(self, audio_path, label_path):
        ...
        '''
        lines = np.load(label_path).tolist()
        i = 0
        for line in lines:
            i += 1
            if float(line) != 0 and i < n_frames:
