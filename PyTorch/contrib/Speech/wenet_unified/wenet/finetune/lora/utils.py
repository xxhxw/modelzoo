...
def reinit_lora_modules(name, module, init_config, **kwargs):
    ...
    if init_config.mode == "simple":
        ...
        U, S, V = torch.svd_lowrank(grads.sdaa().float(), q=4 * lora_r, niter=4)
        V = V.T
        # set direction
        ...
        if init_config.scale == "gd":
            ...
            B = B * avg_s
            A = A * avg_s
        module.lora_B = torch.nn.Parameter(B.contiguous().sdaa())
        module.lora_A = torch.nn.Parameter(A.contiguous().sdaa())
