...
if command -v teco-smi &> /dev/null; then
  echo "Running teco-smi -L and wc -l to count GPUs..."
  num_gpus=$(teco-smi -L | tee /dev/stderr | wc -l) # 打印teco-smi -L的输出同时计算行数
...
echo "SDAA_VISIBLE_DEVICES is ${SDAA_VISIBLE_DEVICES}"
...
data=/data/datasets/20241122/AISHELL-1
...
#    autograd error
train_config=../s0/conf/train_u2++_conformer.yaml
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  mkdir -p $dir
  num_gpus=$(echo $SDAA_VISIBLE_DEVICES | awk -F "," '{print NF}')
