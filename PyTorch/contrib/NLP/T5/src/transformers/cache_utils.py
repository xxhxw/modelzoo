...
class OffloadedCache(DynamicCache):
    ...
    def __init__(self) -> None:
        ...
        self.prefetch_stream = torch.sdaa.Stream()
        self.beam_idx = None  # used to delay beam search operations

    def prefetch_layer(self, layer_idx: int):
        "Starts prefetching the next layer cache"
        if layer_idx < len(self):
            with torch.sdaa.stream(self.prefetch_stream):
...
class HQQQuantizedCache(QuantizedCache):
    ...
    def _quantize(self, tensor, axis):
        ...
        self.quantizer.sdaa(qtensor, meta=meta, device=self.device)  # Move to device and cast to dtype
        return qtensor, meta
...
class OffloadedStaticCache(StaticCache):
    ...
    def __init__(
        ...
        self._prefetch_stream = torch.sdaa.Stream() if self.device.type == "sdaa" else None
    ...
    def update(
        ...
        if layer_idx == 0:
            ...
            if self._prefetch_stream is not None:
                torch.sdaa.default_stream(self.device).wait_stream(self._prefetch_stream)
    ...
    def _prefetch_layer(self, layer_idx: int) -> None:
        ...
        if self._prefetch_stream is not None:
            with torch.sdaa.stream(self._prefetch_stream):
