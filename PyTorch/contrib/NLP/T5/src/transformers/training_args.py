...
class TrainingArguments:
    ...
    Parameters:
        ...
        ddp_backend (`str`, *optional*):
            The backend to use for distributed training. Must be one of `"tccl"`, `"mpi"`, `"ccl"`, `"gloo"`, `"hccl"`.
    ...
    ddp_backend: Optional[str] = field(
        ...
        metadata={
            ...
            "choices": ["tccl", "gloo", "mpi", "ccl", "hccl", "cncl", "mccl"],
        },
    )
        ...
        if self.framework == "pt" and is_torch_available():
            ...
            if self.bf16 or self.bf16_full_eval:
                if self.use_cpu and not is_torch_bf16_cpu_available() and not is_torch_xla_available():
                    ...
                    if torch.sdaa.is_available() and not is_torch_bf16_gpu_available():
                        # gpu
                        raise ValueError(
    ...
    def _setup_devices(self) -> "torch.device":
        ...
        if is_torch_xla_available():
            ...
            if is_torch_sdaa_available():
                device = torch.device(
                    "sdaa:0" if torch.sdaa.is_available() else os.environ.get("ACCELERATE_TORCH_DEVICE", "cpu")
                )
                # Sometimes the line in the postinit has not been run before we end up here, so just checking we're not at
                # the default value.
                self._n_gpu = torch.sdaa.device_count()
                if device.type == "sdaa":
                    torch.sdaa.set_device(device)
            elif self.use_cpu:
