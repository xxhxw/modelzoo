...
def is_torch_bf16_gpu_available():
    ...
    return torch.sdaa.is_available() and torch.sdaa.is_bf16_supported()
...
def is_torch_tf32_available():
    ...
    if not torch.sdaa.is_available() or torch.version.cuda is None:
        return False
    if torch.sdaa.get_device_properties(torch.sdaa.current_device()).major < 8:
        return False
...
def is_torch_xla_available(check_is_tpu=False, check_is_gpu=False):
    ...
    if check_is_gpu:
        return torch_xla.runtime.device_type() in ["GPU", "sdaa"]
    elif check_is_tpu:
...
def is_flash_attn_2_available():
    ...
    if not (torch.sdaa.is_available() or is_torch_mlu_available()):
        return False
