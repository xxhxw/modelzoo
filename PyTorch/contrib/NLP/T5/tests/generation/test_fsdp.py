...
if is_torch_available():
    ...
    def manage_process_group(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def wrapped(*args: Any, **kwargs: Any) -> Any:
            torch.distributed.init_process_group(world_size=torch.sdaa.device_count())
            try:
    ...
    def fsdp_generate():
        torch.sdaa.set_device(device := torch.device(rank := torch.distributed.get_rank()))
    ...
    def fsdp2_generate():
        ...
        mesh = init_device_mesh("sdaa", (torch.distributed.get_world_size(),))
...
class TestFSDPGeneration(TestCasePlus):
    ...
    def test_fsdp_generate(self):
        distributed_args = f"""--nproc_per_node={torch.sdaa.device_count()}
