...
class Mamba2IntegrationTest(unittest.TestCase):
    ...
    def test_mamba2_mixer_train_vs_eval_equivalence(self):
        ...
        with torch.amp.autocast(device_type="sdaa", dtype=dtype):
            with torch.no_grad():
                mixer = Mamba2Mixer(config, layer_idx=0).to("sdaa")
                hidden_states = torch.rand(size=(B, T, D), dtype=dtype, device="sdaa")
