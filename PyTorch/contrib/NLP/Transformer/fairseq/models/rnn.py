...
class RNNEncoder(FairseqEncoder):
    ...
    def forward(
        ...
        packed_outs = packed_outs.to(device_old)
        ...
        if self.rnn_type == "lstm":
            final_hiddens = self.reshape_state(hidden[0].to(device_old), bsz)
            final_cells = self.reshape_state(hidden[1].to(device_old), bsz)
        else:
...
class Attention(nn.Module):
    ...
    def compute_context(
        ...
        softmax_attn = torch.nn.functional.softmax(attn_score+1e-9, dim=1).unsqueeze(1)  # alignment vector alpha
        if self.attention_type in ["luong-dot", "luong-general", "luong-concat"]:
...
class RNNDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features(
        ...
        if self.rnn_type == "gru":
            hidden = prev_hiddens.to("cpu")
        else:
            hidden = (prev_hiddens.to("cpu"), prev_cells.to("cpu"))
        ...
        for j in range(seqlen):
            ...
            if self.attention_type in ["luong-dot", "luong-general", "luong-concat"]:
                ...
                device_old = input.device
                input = input.to("cpu")
                self.layers.to("cpu")
                target_state, hidden = self.layers(input.unsqueeze(0), hidden)
                target_state = target_state.to(device_old)
                ...
                out= out.to(device_old)

            # save state for next time step
            if self.rnn_type == "gru":
                prev_hiddens, prev_cells = hidden.to(device_old), None
            else:
                prev_hiddens, prev_cells = hidden[0].to(device_old), hidden[1].to(device_old)
