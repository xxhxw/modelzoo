...
def move_to_cuda(sample, device=None):
    device = device or torch.sdaa.current_device()
...
def multi_tensor_total_norm(grads, chunk_size=2048 * 32) -> torch.Tensor:
    ...
    for device in per_device_grads.keys():
        ...
        if device.type == "cuda":
            ...
            with torch.sdaa.device(device):
                norm = multi_tensor_l2norm(
                    chunk_size, has_inf, [cur_device_grads], False
                )
            norms.append(norm[0].to(torch.sdaa.current_device()))
        else:
...
def get_rng_state():
    ...
    if torch.sdaa.is_available():
        state["cuda_rng_state"] = torch.sdaa.get_rng_state()
    return state
...
def set_rng_state(state):
    ...
    if torch.sdaa.is_available():
        torch.sdaa.set_rng_state(state["cuda_rng_state"])
...
class CudaEnvironment(object):
    def __init__(self):
        ...
        prop = torch.sdaa.get_device_properties("sdaa:{}".format(cur_device))
        self.name = prop.name
