...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        self.sdaa = torch.sdaa.is_available() and not cfg.common.cpu and not self.tpu
        if self.sdaa:
        ...
        if self.sdaa and self.pipeline_model_parallel:
        ...
        if self.sdaa and self.data_parallel_world_size > 1:
            self._grad_norm_buf = torch.DoubleTensor(self.data_parallel_world_size).sdaa()
    ...
    def _build_optimizer(self):
        ...
        if self.is_fsdp and self.cfg.common.fp16:
            ...
            # if self.cuda and torch.sdaa.get_device_capability(0)[0] < 7:
            #     logger.info(
            ...
            # if self.sdaa and torch.sdaa.get_device_capability(0)[0] >= 7:
            #     logger.info(
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            try:
                ...
                if self.sdaa and self.get_num_updates() == 0:
        ...
        try:
            ...
            grad_norm = torch.tensor(0.0).sdaa()
            self.zero_grad()
        except RuntimeError as e:
            if "out of memory" in str(e):
                self._log_oom(e)
        ...
        if not overflow or self.cfg.distributed_training.ddp_backend == "slowmo":
            ...
            if self.tpu:
                ...
                if self.sdaa and self.cuda_env is not None:
                    # log minimum free memory over the iteration
                    gb_used = torch.sdaa.max_memory_allocated() / 1024 / 1024 / 1024
    ...
    def clip_grad_norm(self, clip_norm):
        def agg_norm_fn(total_norm):
            total_norm = total_norm.sdaa().float() ** 2
    ...
    def _log_oom(self, exc):
        ...
        logger.warning(msg)
        if torch.sdaa.is_available() and hasattr(torch.sdaa, "memory_summary"):
            for device_idx in range(torch.sdaa.device_count()):
                logger.warning(torch.sdaa.memory_summary(device=device_idx))
        sys.stderr.flush()
    ...
    def _check_grad_norms(self, grad_norm):
        """Check that grad norms are consistent across workers."""
        # print(self._grad_norm_buf.shape,self.data_parallel_rank)
        if self._grad_norm_buf is not None:
            self._grad_norm_buf.zero_()
            # print(self._grad_norm_buf.shape,self.data_parallel_rank)

            self._grad_norm_buf[self.data_parallel_rank] = grad_norm.item()
