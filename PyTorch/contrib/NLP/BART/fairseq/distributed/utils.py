    ...
    if cfg.pipeline_model_parallel:
        ...
        with open_dict(cfg):
            cfg.distributed_num_procs = min(
                torch.sdaa.device_count(), cfg.distributed_world_size
            )
    else:
        if cfg.device_id > 0:
            logger.info(
            ...
            torch.sdaa.set_device(cfg.device_id)
...
def _infer_torch_distributed_launch_init(cfg: DistributedTrainingConfig):
    ...
    cfg.device_id = cfg.distributed_rank % torch.sdaa.device_count()
    ...
    if node_list is not None:
        try:
            ...
            if ntasks_per_node == 1:
                gpus_per_node = torch.sdaa.device_count()
...
def _infer_single_node_init(cfg: DistributedTrainingConfig):
    assert (
        cfg.distributed_world_size <= torch.sdaa.device_count()
    ), f"world size is {cfg.distributed_world_size} but have {torch.sdaa.device_count()} available devices"
    ...
    if not cfg.common.tpu:
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            ...
            if torch.sdaa.is_available():
                dist.all_reduce(torch.zeros(1).sdaa())
...
def distributed_main(i, main, cfg: FairseqConfig, kwargs):
    ...
    if torch.sdaa.is_available() and not cfg.common.cpu and not cfg.common.tpu:
        torch.sdaa.set_device(cfg.distributed_training.device_id)
...
def all_gather_list(data, group=None, max_size=16384):
    ...
    if (
        ...
        # print(buffer_size,max_size,world_size)
        all_gather_list._buffer = torch.ByteTensor(buffer_size).sdaa()
...
def broadcast_tensors(
    ...
    if dist_device is None:
        if torch.distributed.get_backend(group) == "tccl":
            dist_device = torch.device("sdaa")
        else:
