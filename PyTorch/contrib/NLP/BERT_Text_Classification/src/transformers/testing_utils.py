...
def require_torch_non_multi_gpu(test_case):
    ...
    return unittest.skipUnless(torch.sdaa.device_count() < 2, "test requires 0 or 1 GPU")(test_case)
...
def require_torch_up_to_2_gpus(test_case):
    ...
    return unittest.skipUnless(torch.sdaa.device_count() < 3, "test requires 0 or 1 or 2 GPUs")(test_case)
...
if is_torch_available():
    ...
    if "TRANSFORMERS_TEST_DEVICE" in os.environ:
        ...
        if torch_device == "sdaa" and not torch.sdaa.is_available():
            raise ValueError(
...
def require_torch_gpu(test_case):
    ...
    return unittest.skipUnless(torch_device == "sdaa", "test requires CUDA")(test_case)
...
def preprocess_string(string, skip_cuda_tests):
    ...
    for i, codeblock in enumerate(codeblocks):
        ...
        if (
            ...
            and re.search(r"sdaa|to\(0\)|device=0", codeblock)
            and skip_cuda_tests
        ):
            is_cuda_found = True
            break
...
if is_torch_available():
    ...
    # testing.
    BACKEND_MANUAL_SEED = {"sdaa": torch.sdaa.manual_seed, "cpu": torch.manual_seed, "default": torch.manual_seed}
    BACKEND_EMPTY_CACHE = {"sdaa": torch.sdaa.empty_cache, "cpu": None, "default": None}
    BACKEND_DEVICE_COUNT = {"sdaa": torch.sdaa.device_count, "cpu": lambda: 0, "default": lambda: 1}
else:
