...
class Bnb4BitTest(Base4bitTest):
    ...
    def test_device_assignment(self):
        ...
        if torch.sdaa.is_available():
            ...
            self.model_4bit.to("sdaa")
            self.assertEqual(self.model_4bit.device.type, "sdaa")
            self.assertAlmostEqual(self.model_4bit.get_memory_footprint(), mem_before)
    ...
    def test_device_and_dtype_assignment(self):
        ...
        if version.parse(importlib.metadata.version("bitsandbytes")) < version.parse("0.43.2"):
            ...
            with self.assertRaises(ValueError):
                # Tries with a `device`
                self.model_4bit.to(torch.device("sdaa:0"))

            with self.assertRaises(ValueError):
                # Tries with `cuda`
                self.model_4bit.sdaa()
...
class Bnb4BitTestTraining(Base4bitTest):
    ...
    def test_training(self):
        ...
        if torch.sdaa.is_available():
            self.assertEqual(set(model.hf_device_map.values()), {torch.sdaa.current_device()})
        else:
