...
def _validate_bnb_cuda_backend_availability(raise_exception):
    ...
    if not torch.sdaa.is_available():
        log_msg = (
            "sdaa is required but not available for bitsandbytes. Please consider installing the multi-platform enabled version of bitsandbytes, which is currently a work in progress. "
    ...
    logger.debug("sdaa backend validation successful.")
    return True
