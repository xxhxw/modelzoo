    ...
    def _check_and_enable_flash_attn_2(
        ...
        if not is_flash_attn_2_available():
            ...
            if torch.version.cuda:
                ...
                elif not torch.sdaa.is_available():
                    raise ValueError(
        ...
        # The check `torch.empty(0).device.type != "sdaa"` is needed as the model may be initialized after `torch.set_default_device` has been called,
        # or the model may be initialized under the context manager `with torch.device("sdaa"):`.
        if check_device_map and device_map is None and torch.empty(0).device.type != "sdaa":
            if torch.sdaa.is_available():
                logger.warning_once(
                    "You are attempting to use Flash Attention 2.0 with a model not initialized on GPU. Make sure to move the model to GPU"
                    " after initializing it on CPU with `model.to('sdaa')`."
                )
            else:
                raise ValueError(
    ...
    def sdaa(self, *args, **kwargs):
        ...
        if getattr(self, "quantization_method", None) == QuantizationMethod.BITS_AND_BYTES:
            if getattr(self, "is_loaded_in_8bit", False):
                raise ValueError(
                    "Calling `sdaa()` is not supported for `8-bit` quantized models. "
                    " Please use the model as it is, since the model has already been set to the correct devices."
                )
            elif version.parse(importlib.metadata.version("bitsandbytes")) < version.parse("0.43.2"):
                raise ValueError(
                    "Calling `sdaa()` is not supported for `4-bit` quantized models with the installed version of bitsandbytes. "
                    f"The current device is `{self.device}`. If you intended to move the model, please install bitsandbytes >= 0.43.2."
                )
        else:
            return super().sdaa(*args, **kwargs)
        ...
        **kwargs,
        ...
        Parameters:
            ...
            device_map (`str` or `Dict[str, Union[int, str, torch.device]]` or `int` or `torch.device`, *optional*):
                ...
                same device. If we only pass the device (*e.g.*, `"cpu"`, `"sdaa:1"`, `"mps"`, or a GPU ordinal rank
