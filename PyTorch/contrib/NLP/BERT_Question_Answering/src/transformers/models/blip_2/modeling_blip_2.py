...
class Blip2ForConditionalGeneration(Blip2PreTrainedModel, GenerationMixin):
    ...
    def _preprocess_accelerate(self):
        ...
        if len(hf_device_map) > 1 and "language_model" not in hf_device_map and torch.sdaa.device_count() > 1:
    ...
    def forward(
        ...
        ```python
        >>> prompt = "Question: how many cats are there? Answer:"
        >>> inputs = processor(images=image, text=prompt, return_tensors="pt").to(device="sdaa", dtype=torch.float16)
        ...
        >>> inputs = processor(images=image, text=prompt, return_tensors="pt").to(device="sdaa", dtype=torch.bfloat16)
