    ...
    def _load_rng_state(self, checkpoint):
        ...
        if torch.sdaa.is_available():
            if self.args.parallel_mode == ParallelMode.DISTRIBUTED:
                torch.sdaa.random.set_rng_state_all(checkpoint_rng_state["sdaa"])
            else:
                try:
                    torch.sdaa.random.set_rng_state(checkpoint_rng_state["sdaa"])
    ...
    def _save_rng_state(self, output_dir):
        ...
        if torch.sdaa.is_available():
            if self.args.parallel_mode == ParallelMode.DISTRIBUTED:
                ...
                rng_states["sdaa"] = torch.sdaa.random.get_rng_state_all()
            else:
                rng_states["sdaa"] = torch.sdaa.random.get_rng_state()
