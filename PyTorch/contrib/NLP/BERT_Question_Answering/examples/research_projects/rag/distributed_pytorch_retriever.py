...
class RagPyTorchDistributedRetriever(RagRetriever):
    ...
    def init_retrieval(self, distributed_port: int):
        ...
        # tccl backend doesn't support gather/scatter operations while gloo
        # is too slow to replace tccl for the core gpu communication
        if dist.is_initialized():
            logger.info("dist initialized")
            # needs to be set manually
            os.environ["GLOO_SOCKET_IFNAME"] = self._infer_socket_ifname()
            # avoid clash with the tccl port
