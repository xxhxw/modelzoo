...
class MockCudaOOMCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        if state.train_batch_size >= self.batch_size_limit:
            raise RuntimeError("sdaa out of memory.")
...
class TorchTracemalloc:
    ...
    def __exit__(self, *exc):
        ...
        if torch.sdaa.is_available():
            ...
            self.peak = torch.sdaa.max_memory_allocated()
...
class TrainerIntegrationTest(TestCasePlus, TrainerIntegrationCommon):
    ...
    def test_resume_training_with_randomness(self):
        ...
        random_torch = not torch.sdaa.is_available() or torch.sdaa.device_count() <= 1
    ...
    def test_torchdynamo_memory(self):
        ...
        a = torch.ones(1024, 1024, device="sdaa", requires_grad=True)
        a.grad = None
        ...
        orig_loss = trainer.training_step(mod, {"x": a})
        orig_peak_mem = torch.sdaa.max_memory_allocated()
        torchdynamo.reset()
        del trainer
        ...
        loss = trainer.training_step(mod, {"x": a})
        peak_mem = torch.sdaa.max_memory_allocated()
        torchdynamo.reset()
        del trainer
