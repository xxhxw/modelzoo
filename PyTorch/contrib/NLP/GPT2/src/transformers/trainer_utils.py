...
class TrainerMemoryTracker:
    ...
    def __init__(self, skip_memory_metrics=False):
        ...
        if is_torch_sdaa_available() or is_torch_mlu_available() or is_torch_musa_available():
            import torch
    ...
    def start(self):
        ...
        if self.torch is not None:
            if torch.sdaa.is_available():
                self.torch.sdaa.reset_peak_memory_stats()
                self.torch.sdaa.empty_cache()
        ...
        if self.torch is not None:
            if torch.sdaa.is_available():
                self.gpu_mem_used_at_start = self.torch.sdaa.memory_allocated()
            elif is_torch_mlu_available():
    ...
    def stop(self, stage):
        ...
        if self.torch is not None:
            if torch.sdaa.is_available():
                self.gpu_mem_used_now = self.torch.sdaa.memory_allocated()
                self.gpu_mem_used_peak = self.torch.sdaa.max_memory_allocated()
            elif is_torch_mlu_available():
