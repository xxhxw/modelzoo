...
import pandas as pd
def extract_losses_from_ncf(log):
    df = pd.read_csv(log)
    epochs = []
    hit_rates = []
    ...
    for i in range(len(df)):
        ...
        hit_rates.append(line["hit_rate"])
    return epochs,hit_rates

def draw_ncf():
    log_file = "train_sdaa_3rd.csv"
    epochs,hit_rates = extract_losses_from_ncf(log_file)
    plt.plot(epochs,hit_rates)
    plt.xlabel('Epoch')
    plt.ylabel('Hit rate')
    plt.title('Hit rates over Iterations') 
    plt.grid(True)
