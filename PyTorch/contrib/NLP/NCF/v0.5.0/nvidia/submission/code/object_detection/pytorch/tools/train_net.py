...
def main():
    ...
    if args.distributed:
        ...
        # Setting seed
        seed_tensor = torch.tensor(0, dtype=torch.float32, device=torch.device("sdaa"))
        ...
        if torch.distributed.get_rank() == 0:
            ...
            seed_tensor = torch.tensor(master_seed, dtype=torch.float32, device=torch.device("sdaa"))
