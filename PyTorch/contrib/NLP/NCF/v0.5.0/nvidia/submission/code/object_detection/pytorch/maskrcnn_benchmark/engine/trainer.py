...
def train_one_epoch(
    ...
    num_pad_batches = 0
    if distributed:
        num_batches_tensor = torch.sdaa.LongTensor([num_batches])
        gathered = [torch.sdaa.LongTensor(1) for i in range(torch.distributed.get_world_size())]
    ...
    def padded_prefetcher(load_iterator, num_pad_batches):
        prefetch_stream = torch.sdaa.Stream()
        pad_batches = []
        ...
        def _prefetch():
            ...
            with torch.sdaa.stream(prefetch_stream):
        ...
        while next_images is not None:
            torch.sdaa.current_stream().wait_stream(prefetch_stream)
        ...
        if iteration % 20 == 0 or iteration == (max_iter - 1):
            logger.info(
                meters.delimiter.join(
                    ...
                    memory=torch.sdaa.max_memory_allocated() / 1024.0 / 1024.0,
                )
            )
    ...
    while arguments["iteration"] < max_iter:
        ...
        if config.DO_ONLINE_MAP_EVAL:
            ...
            print_mlperf(key=mlperf_log.EVAL_TARGET, value={"BBOX": 0.377,
                                                              "SEGM": 0.339})
            map_tensor = torch.tensor((0, 0), dtype=torch.float32, device=torch.device("sdaa"))

            if results: #Rank 0 process
                bbox_map = results['bbox']
                mask_map = results['segm']
                map_tensor = torch.tensor((bbox_map, mask_map), dtype=torch.float32, device=torch.device("sdaa"))
