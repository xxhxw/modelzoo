...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, left_pad=True):
        ...
        transformer_print(key=mlperf_log.MODEL_HP_NUM_HIDDEN_LAYERS, value=args.encoder_layers)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, args):
        ...
        transformer_print(key=mlperf_log.MODEL_HP_RELU_DROPOUT, value=self.relu_dropout)
        self.fuse_dropout_add = args.fuse_dropout_add
        self.fuse_relu_dropout = args.fuse_relu_dropout
        ...
        transformer_print(key=mlperf_log.MODEL_HP_FFN_FILTER_DENSE,
                value={'filter_size':args.encoder_ffn_embed_dim, 'activation':'relu', 'use_bias':True})
        transformer_print(key=mlperf_log.MODEL_HP_FFN_OUTPUT_DENSE,
                value={'hidden_size':self.embed_dim, 'use_bias':True})
        transformer_print(key=mlperf_log.MODEL_HP_LAYER_POSTPROCESS_DROPOUT, value=self.dropout)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, args, no_encoder_attn=False):
        ...
        transformer_print(key=mlperf_log.MODEL_HP_FFN_FILTER_DENSE,
                value={'filter_size':args.decoder_ffn_embed_dim, 'activation':'relu', 'use_bias':True})
        transformer_print(key=mlperf_log.MODEL_HP_FFN_OUTPUT_DENSE,
                value={'hidden_size':self.embed_dim, 'use_bias':True})
        transformer_print(key=mlperf_log.MODEL_HP_LAYER_POSTPROCESS_DROPOUT, value=self.dropout)
        transformer_print(key=mlperf_log.MODEL_HP_NORM, value=self.embed_dim)
