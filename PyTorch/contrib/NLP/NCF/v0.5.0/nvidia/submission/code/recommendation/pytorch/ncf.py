...
def main():
    ...
    print(args.distributed, args.world_size)
    ...
    use_sdaa =  torch.sdaa.is_available()
    ...
    if args.distributed:
        torch.distributed.broadcast(torch.tensor([1], device="sdaa"), 0)
    ...
    train_ratings = torch.load(args.data+'/train_ratings.pt', map_location=torch.device('sdaa:{}'.format(args.local_rank)))
    test_ratings = torch.load(args.data+'/test_ratings.pt', map_location=torch.device('sdaa:{}'.format(args.local_rank)))
    ...
    mat = torch.ByteTensor(nb_users, nb_items).fill_(1).sdaa()
    ...
    if args.distributed:
        optimizer = torch.optim.Adam(model_ddp.parameters(), lr=args.learning_rate, betas=(args.beta1, args.beta2), eps=args.eps)
    else:
    ...
    for epoch in range(args.epochs):
        ...
        epoch_indices = torch.randperm(len(epoch_users), device='sdaa:{}'.format(args.local_rank))
        ...
        for i in qbar:
            ...
            #AMP
            # user = user.to(memory_format=torch.channels_last)
            optimizer.zero_grad()
            if args.AMP:
        ...
        ckpt_path = run_dir+f"/checkpoint_{epoch}.pth"
        torch.save({
            'epoch': epoch,
