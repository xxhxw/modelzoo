...
def main():
    ...
    if args.local_rank == -1 or args.no_cuda:
        args.device = torch.device("sdaa" if torch.sdaa.is_available() and not args.no_cuda else "cpu")
        args.n_gpu = 0 if args.no_cuda else torch.sdaa.device_count()
    else:
        torch.sdaa.set_device(args.local_rank)
        args.device = torch.device("sdaa", args.local_rank)
        args.n_gpu = 1
        torch.distributed.init_process_group(backend="tccl")  # Initializes the distributed backend
