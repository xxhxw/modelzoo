...
class ReformerLayer(nn.Module):
    ...
    def _init_attention_seed(self):
        ...
        if hasattr(torch.sdaa, "default_generators") and len(torch.sdaa.default_generators) > 0:
            # GPU
            device_idx = torch.sdaa.current_device()
            self.attention_seed = torch.sdaa.default_generators[device_idx].seed()
        else:
            # CPU
    ...
    def _init_feed_forward_seed(self):
        ...
        if hasattr(torch.sdaa, "default_generators") and len(torch.sdaa.default_generators) > 0:
            # GPU
            device_idx = torch.sdaa.current_device()
            self.feed_forward_seed = torch.sdaa.default_generators[device_idx].seed()
        else:
            # CPU
