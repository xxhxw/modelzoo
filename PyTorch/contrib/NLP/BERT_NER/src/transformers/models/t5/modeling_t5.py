...
DEPARALLELIZE_DOCSTRING = r"""
    ...
    model.deparallelize()  # Put the model back on cpu and cleans memory by calling torch.sdaa.empty_cache()
    ```
"""
...
class T5Stack(T5PreTrainedModel):
    ...
    def parallelize(self, device_map=None):
        ...
        self.device_map = (
            get_device_map(len(self.block), range(torch.sdaa.device_count())) if device_map is None else device_map
        )
        assert_device_map(self.device_map, len(self.block))
        self.model_parallel = True
        self.first_device = "cpu" if "cpu" in self.device_map.keys() else "sdaa:" + str(min(self.device_map.keys()))
        self.last_device = "sdaa:" + str(max(self.device_map.keys()))
        # Load onto devices
        for k, v in self.device_map.items():
            for layer in v:
                cuda_device = "sdaa:" + str(k)
    ...
    def forward(
        ...
        if self.model_parallel:
            torch.sdaa.set_device(self.first_device)
            self.embed_tokens = self.embed_tokens.to(self.first_device)
        ...
        for i, layer_module in enumerate(self.block):
            ...
            # Model parallel
            if self.model_parallel:
                torch.sdaa.set_device(hidden_states.device)
            ...
            if self.model_parallel:
                for k, v in self.device_map.items():
                    if i == v[-1] and "sdaa:" + str(k) != self.last_device:
                        hidden_states = hidden_states.to("sdaa:" + str(k + 1))
    ...
    def _update_causal_mask(
        ...
        if (
            ...
            and attention_mask.device.type == "sdaa"
...
class T5Model(T5PreTrainedModel):
    ...
    def parallelize(self, device_map=None):
        ...
        self.device_map = (
            get_device_map(len(self.encoder.block), range(torch.sdaa.device_count()))
    ...
    def forward(
        ...
        if self.model_parallel:
            torch.sdaa.set_device(self.decoder.first_device)
...
class T5ForConditionalGeneration(T5PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        # Set device for model parallelism
        if self.model_parallel:
            torch.sdaa.set_device(self.encoder.first_device)
