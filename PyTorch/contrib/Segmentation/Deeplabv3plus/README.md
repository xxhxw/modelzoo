...
## 1. 模型概述
DeepLabv3+是一种用于图像语义分割的先进卷积神经网络架构。它结合了深度可分离卷积和多尺度空洞卷积的空间金字塔池化（ASPP），并采用编码器-解码器结构来提升分割精度和边界细节。通过捕获多尺度上下文信息和增强边界细节，DeepLabv3+在自动驾驶、医学影像分析和遥感影像等领域表现出色。
...
1. 执行以下命令，启动虚拟环境。
``` bash
cd <ModelZoo_path>/PyTorch/contrib/Segmentation/Deeplabv3plus
...
1. 在构建好的环境中，进入训练脚本所在目录。
    ```
    cd <ModelZoo_path>/PyTorch/contrib/Segmentation/Deeplabv3plus/run_scripts
    ```


注：在vaihingen数据集上训练150epoch，4卡DDP，开启amp得到的pth权重文件可在以下链接中下载，如果需要可以直接下载并将其存放到`/experiments/example`目录中，可以直接进行结果测试和可视化。\
\
链接：https://pan.baidu.com/s/19yFlIyUGOlbzPmHvw5LLXQ \
提取码：8wk0 
...
- 单机单SPA训练
    ```
    python run_scripts/run_deeplabv3plus.py --device sdaa --work_dir experiments/deeplabv3plus --distributed False --use_amp True --default_rank 0 --model_name deeplabv3plus --crop_val True --lr 0.01 --crop_size 512 --batch_size 4 --total_epochs 150 --val_epoch 5
    ```
- 单机单卡训练（DDP）
    ```
    python run_scripts/run_deeplabv3plus.py --nproc_per_node=4 --master_port=29505 --device sdaa --work_dir experiments/deeplabv3plus --distributed True --use_amp True --default_rank 0 --model_name deeplabv3plus --crop_val True --lr 0.01 --crop_size 512 --batch_size 4 --total_epochs 150 --val_epoch 5
    ```
...
- 单机单SPA测试
    ```
    python run_scripts/run_deeplabv3plus.py --distributed False --device sdaa --ckpt experiments/example/best_deeplabv3plus_vaihingen.pth --use_amp True --test_only True --default_rank 0 --model_name deeplabv3plus --crop_val True --crop_size 512
    ```
- 单机单卡测试
    ```
    python run_scripts/run_deeplabv3plus.py --nproc_per_node=4 --master_port=29505 --distributed True --device sdaa --ckpt experiments/example/best_deeplabv3plus_vaihingen.pth --use_amp True --test_only True --default_rank 0 --model_name deeplabv3plus --crop_val True --crop_size 512
    ```
...
参数名 | 解释 | 样例
-----------------|-----------------|-----------------
ckpt | 用于模型推理的checkpoint文件 | --ckpt experiments/example/best_deeplabv3plus_vaihingen.pth
...
model_name | 模型名称 | --model_name /deeplabv3plus
...
|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
|SDAA|1| Deeplabv3plus |是|4|512*512| 15 img/s|
...
| 目标类别 |IoU  |
|:-:|:-:|
| Impervious surfaces | 81.79 |
| Building | 87.95 |
| Low vegetation | 65.02 |
| Tree | 76.99 |
| Car | 60.25 |
