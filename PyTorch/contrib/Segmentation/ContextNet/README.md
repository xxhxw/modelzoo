...
文章链接：https://arxiv.org/pdf/1805.04554
...
1. 执行以下命令，启动虚拟环境。
``` bash
cd <ModelZoo_path>/PyTorch/contrib/Segmentation/ContextNet
...
\
链接：https://pan.baidu.com/s/1I547jygfGuHlyWKBqOL_Mw \
提取码：bcrj 
...
- 单机单SPA训练
    ```
    python run_scripts/run_contextnet.py --device sdaa --work_dir experiments/contextnet --distributed False --use_amp True --default_rank 0 --model_name contextnet --crop_val True --lr 0.0005 --optimizer adam --crop_size 512 --batch_size 4 --total_epochs 150 --val_epoch 5
    ```
- 单机单卡训练（DDP）
    ```
    python run_scripts/run_contextnet.py --nproc_per_node=4 --master_port=29505 --device sdaa --work_dir experiments/contextnet --distributed True --use_amp True --default_rank 0 --model_name contextnet --crop_val True --lr 0.0005 --optimizer adam --crop_size 512 --batch_size 4 --total_epochs 150 --val_epoch 5
    ```
...
- 单机单SPA测试
    ```
    python run_scripts/run_contextnet.py --distributed False --device sdaa --ckpt experiments/example/best_contextnet_vaihingen.pth --use_amp True --test_only True --default_rank 0 --model_name contextnet --crop_val True --crop_size 512
    ```
- 单机单卡测试
    ```
    python run_scripts/run_contextnet.py --nproc_per_node=4 --master_port=29505 --distributed True --device sdaa --ckpt experiments/example/best_contextnet_vaihingen.pth --use_amp True --test_only True --default_rank 0 --model_name contextnet --crop_val True --crop_size 512
    ```
...
参数名 | 解释 | 样例
-----------------|-----------------|-----------------
ckpt | 用于模型推理的checkpoint文件 | --ckpt experiments/example/best_contextnet_vaihingen.pth
...
model_name | 模型名称 | --model_name /contextnet
...
|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
|SDAA|1| ContextNet |是|4|512*512| 60 img/s|
...
| 目标类别 | IoU |
|:-:|:-:|
| Impervious surfaces | 75.01 |
| Building | 78.37 |
| Low vegetation | 62.85 |
| Tree | 75.07 |
| Car | 30.53 |
