...
logger = get_logger('UNetTrainer',log_file='scripts/train_sdaa_3rd.log')
...
def create_trainer(config):
    ...
    if torch.sdaa.is_available():
        if distributed:
            ...
            logger.info(f'Using {torch.sdaa.device_count()} GPUs for prediction')
        else:
            device = torch.device(f"sdaa:{local_rank}")
            torch.sdaa.set_device(device)
            model = model.to(device)
    ...
    #     model = model.cuda()
    # if torch.sdaa.device_count() > 1 and not config['device'] == 'cpu':
    #     # model = nn.DataParallel(model)   
    #     model = nn.parallel.DistributedDataParallel(model)# , find_unused_parameters=True) 
    #     logger.info(f'Using {torch.sdaa.device_count()} GPUs for prediction')
    # if torch.sdaa.is_available() and not config['device'] == 'cpu':
    #     model = model.sdaa()
...
class UNetTrainer:
    ...
    def __init__(self, model, optimizer, lr_scheduler, loss_criterion, eval_criterion, loaders, checkpoint_dir,
        ...
        self.amp_bool = False  # 是否使用混合精度计算，默认否
        logger.info(model)
    ...
    def train(self):
        ...
        for t in self.loaders['train']:
            ...
            if self.amp_bool:
                ...
                loss.backward()
                self.optimizer.step()
            logger.info('Training stats. Loss: {}'.format(loss.item()))
            total_training_time = time.time() - start_time
            print("total_training_time:11111",total_training_time)
            if total_training_time>=7200:
                sys.exit(1)
    ...
    def _split_training_batch(self, t):
        def _move_to_gpu(input):
            if isinstance(input, tuple) or isinstance(input, list):
                ...
                if torch.sdaa.is_available():
                    input = input.sdaa(non_blocking=True)
                return input
