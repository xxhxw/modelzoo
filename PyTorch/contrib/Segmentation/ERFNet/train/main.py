...
def train(args, model, enc=False):
    ...
    # 创建一个文件处理器，将日志写入指定文件
    file_handler = logging.FileHandler('../scripts/train_sdaa_3rd.log')
    ...
    # if (enc):
    #     automated_log_path = savedir + "/train_sdaa_3rd.log"
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        for step, (images, labels) in enumerate(loader):
            ...
            if step % 10 == 0 and args.local_rank == 0:
                logger.info(
                    "Epoch: {:d}/{:d} || Iters: {:d}/{:d} ||Loss: {:.4f}".format(
                        epoch, args.num_epochs, step % len(loader), len(loader),loss.item()))
                print(
                    "------------------------------Epoch: {:d}/{:d} || Iters: {:d}/{:d} ||Loss: {:.4f}-------------------------------".format(
                        epoch, args.num_epochs, step % len(loader), len(loader),loss.item()))
            # if (doIouTrain):
...
def main(args):
    ...
    model = model_file.Net(NUM_CLASSES).to(args.device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datadir', default= "/data/datasets/cityscapes/")
    parser.add_argument('--height', type=int, default=512)
    parser.add_argument('--amp_bool', type=int, default=True)
    parser.add_argument('--time_out', type=int, default=7200)
    ...
    parser.add_argument('--savedir', default="../scripts/")
