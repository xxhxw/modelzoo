...
def allocate_buffers(engine):
    h_input = sdaa.pagelocked_empty(
            trt.volume(engine.get_binding_shape(0)), dtype=in_datatype)
    print(engine.get_binding_shape(0))
    d_input = sdaa.mem_alloc(h_input.nbytes)
    h_outputs, d_outputs = [], []
    n_outs = 1
    for i in range(n_outs):
        h_output = sdaa.pagelocked_empty(
                trt.volume(engine.get_binding_shape(i+1)),
                dtype=out_datatype)
        d_output = sdaa.mem_alloc(h_output.nbytes)
...
def main():
    if args.command == 'compile':
        ...
        sdaa.memcpy_htod_async(d_input, h_input, stream)
        context.execute_async(
            bindings=bds, stream_handle=stream.handle)
        for h_output, d_output in zip(h_outputs, d_outputs):
            sdaa.memcpy_dtoh_async(h_output, d_output, stream)
        stream.synchronize()
