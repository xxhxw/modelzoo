...
class Metrics(object):
    ...
    def __init__(self, n_classes, lb_ignore=255):
        ...
        self.confusion = torch.zeros((n_classes, n_classes)).sdaa().detach()
...
class MscEvalV0(object):
    ...
    def __call__(self, net, dl):
        ...
        hist = torch.zeros(n_classes, n_classes).sdaa().detach()
        ...
        for i, (imgs, label) in diter:
            imgs = self.sp(imgs)
            N, _, H, W = imgs.shape
            label = label.squeeze(1).sdaa()
            size = label.size()[-2:]
            probs = torch.zeros(
                    (N, n_classes, *size),
                    dtype=torch.float32).sdaa().detach()
...
class MscEvalCrop(object):
    ...
    def pad_tensor(self, inten):
        ...
        outten = torch.zeros(N, C, padH, padW).sdaa()
    ...
    def crop_eval(self, net, im, n_classes):
        ...
        prob = torch.zeros(N, n_classes, H, W).sdaa()
    ...
    def __call__(self, net, dl):
        ...
        for i, (imgs, label) in diter:
            ...
            probs = torch.zeros((N, n_classes, *size)).sdaa()
