...
class LossHistory():
    ...
    def append_f_score(self, epoch, f_score):
        if not os.path.exists(self.log_dir):
            os.makedirs(self.log_dir)

        self.f_scores.append(f_score)

        with open(os.path.join(self.log_dir, "epoch_f_score.txt"), 'a') as f:
            f.write(str(f_score))
            f.write("\n")
    ...
    def f_score_plot(self):
        iters = range(len(self.f_scores))

        plt.figure()
        plt.plot(iters, self.f_scores, 'red', linewidth=2, label='train f_score')
        try:
            if len(self.f_scores) < 25:
                num = 5
            else:
                num = 15

            plt.plot(iters, scipy.signal.savgol_filter(self.f_scores, num, 3), 'green', linestyle='--', linewidth=2,
        ...
        plt.savefig(os.path.join(self.log_dir, "epoch_f_score.png"))
