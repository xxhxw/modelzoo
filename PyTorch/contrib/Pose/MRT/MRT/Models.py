...
class Transformer(nn.Module):
    ...
    def forward(self, src_seq, trg_seq, input_seq, use=None):
        ...
        _temp_a = input_seq.unsqueeze(1)
        temp_a=torch.cat([_temp_a for i in range(input_seq.shape[1])], dim=1)
        
        # temp_b=input_seq[:,:,-1:,:].unsqueeze(2).repeat(1,1,input_seq.shape[1],1,1)
        _temp_b = input_seq[:,:,-1:,:].unsqueeze(2)
        temp_b=torch.cat([_temp_b for i in range(input_seq.shape[1])], dim=2)
