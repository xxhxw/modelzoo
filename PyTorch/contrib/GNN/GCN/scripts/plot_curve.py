...
def plot_cur(log_name):
    ...
    # 读取文件内容并解析数据
    with open(log_name+".txt", 'r') as file:
        for line in file:
    ...
    plt.legend()
    plt.savefig(f'train_sdaa_3rd_loss_{name}.png')  # 保存Loss曲线图片

    plt.figure(figsize=(10, 6))
    plt.plot(range(acc_list.shape[0]), acc_list, label='acc')
    plt.xlabel('Iteration')
    plt.ylabel('Acc')
    plt.title('Training acc-cora')
    plt.legend()
    plt.savefig(f'train_sdaa_3rd_acc_{name}.png')  # 保存acc曲线图片
                
                
log_folder = ["train_model_log_cora"]
