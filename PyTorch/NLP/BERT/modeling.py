...
class Classifier(nn.Module):
    ...
    def forward(self, x, mask_cls):
        ...
        if self.linear1.weight.data.type() != 'torch.FloatTensor':
            self.linear1.weight = torch.nn.Parameter(self.linear1.weight.data.float())

        h = self.linear1(x).squeeze(-1)
        sent_scores = self.sigmoid(h) * mask_cls.float()
        return sent_scores

class BertForTextSummarization(BertPreTrainedModel):
    def __init__(self, config):        
        super(BertForTextSummarization, self).__init__(config)
        self.bert = BertModel(config)
        self.encoder = Classifier(config.hidden_size)

    def forward(self, x, segs, clss, mask, mask_cls, sentence_range=None):

        encoded_layers, _ = self.bert(x, segs, mask)
        # breakpoint()
