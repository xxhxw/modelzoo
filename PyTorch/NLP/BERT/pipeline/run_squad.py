...
def main():
    ...
    if args.do_train:
        if args.fp16:
            if args.device == "cuda":
                ...
                optimizer = optim.FusedAdam(optimizer_grouped_parameters,
                                lr=args.learning_rate,
                                bias_correction=False)
    ...
    if args.do_train:
        ...
        train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=args.train_batch_size * n_gpu, drop_last=True)
        if is_main_process():
